...
import {
  ...
  listSubjectKeywordsAction,
  type SubjectKeywordRow,
...
export default function DriveMaterialsPanel({ subjects }: { subjects: AdminSubject[] }) {
  ...
  const [keywordRows, setKeywordRows] = useState<SubjectKeywordRow[]>([]);

  // 과목의 키워드 — Drive 폴더는 과목 → 키워드다. 단원은 여기서 고르지 않는다(회차 구성의 일).
  useEffect(() => {
    if (!subjectId) return;
    let cancelled = false;
    listSubjectKeywordsAction(subjectId).then((rows) => {
  ...
  return (
    <div className="max-w-[720px] px-8 py-8">
      ...
      <p className="text-[13px] text-grey-500 mb-5">
        Drive 구조는 과목 → 키워드 → 파일입니다. 키워드 폴더에 올린 PDF·영상을 자료로 가져옵니다. 업로드만으로는 공개되지 않고,
