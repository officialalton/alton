...
import { runGeneratePayouts } from "./payouts-cron";
...
// 실행 로직(runGeneratePayouts)은 "use server"가 아닌 ./payouts-cron 모듈에 있다 — cron 전용
// 진입점(generatePayoutsAsCron)을 이 파일에 두면 Server Action으로 등록되어 인증 없이 직접
// 호출 가능해지기 때문에, 그 함수는 반드시 payouts-cron.ts에만 있어야 한다.
