...
//
// 예외를 던지지 않고 { ok, error } 결과값으로 반환한다 — Next.js는
// Server Action에서 던져진 에러를 production에서 digest만 남기고
// 클라이언트에는 마스킹된 일반 메시지("Minified React error #441")만
// 전달한다(실측 확인, 2026-09-01). 관리자가 실제로 원인(예: 이미 진행
// 중인 프로비저닝, Directory API 409 충돌)을 화면에서 보려면 기대되는
// 실패는 던지지 않고 반환값으로 모델링해야 한다(Next.js 공식 권장 패턴).
export async function startTeacherWorkspaceProvisioning(params: {
  ...
  try {
    ...
    if (created.conflict) {
      // unmanaged/충돌 계정은 자동 병합하지 않는다. (이전에는 이 경로가
      // 조용히 성공한 것처럼 반환돼 관리자가 알아챌 방법이 없었다 —
      // 2026-09-01 발견·수정.)
      const { error } = await supabase.rpc("record_workspace_creation_failed", {
        p_provisioning_id: provisioning.id,
      ...
      return {
        ok: false,
        error: "이미 Google Workspace에 존재하는 이메일입니다(수동 검토 필요) — 자동 생성하지 않았습니다.",
      };
    }
    ...
    return { ok: false, error: e instanceof Error ? e.message : "프로비저닝 처리 중 오류가 발생했습니다." };
  }
}
...
export async function suspendTeacher(teacherId: string, reason: string): Promise<ActionResult> {
  try {
    ...
    return { ok: false, error: e instanceof Error ? e.message : "중단 처리 중 오류가 발생했습니다." };
  }
}
...
export async function reactivateTeacher(params: {
  ...
  try {
    ...
    return { ok: false, error: e instanceof Error ? e.message : "복귀 처리 중 오류가 발생했습니다." };
  }
}

export type ChecklistResult =
  | { ok: true; data: { condition: string; satisfied: boolean; evidence_at: string | null }[] }
...
export async function getTeacherActivationChecklist(teacherId: string): Promise<ChecklistResult> {
  try {
    ...
    if (error) throw new Error(error.message);
    return { ok: true, data: data as { condition: string; satisfied: boolean; evidence_at: string | null }[] };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "체크리스트 조회 중 오류가 발생했습니다." };
  }
}
