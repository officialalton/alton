    ...
    <div>
      ...
      <div className="mb-2" data-testid="figure-section">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-1.5">
          {(["parallel_transversal", "triangle", "circle", "polygon", "solid", "plane", "data"] as const).map((kind) => (
            <button
              key={kind}
              type="button"
            ...
            >
              {figureBusy ? "만드는 중…" : kind === "plane" ? "AI로 좌표평면 데이터 만들기" : kind === "data" ? "AI로 표·그래프 데이터 만들기" : kind === "triangle" ? "AI로 도형 데이터 만들기(삼각형)" : kind === "circle" ? "AI로 도형 데이터 만들기(원)" : kind === "polygon" ? "AI로 도형 데이터 만들기(사각형·다각형)" : kind === "solid" ? "AI로 도형 데이터 만들기(입체)" : "AI로 도형 데이터 만들기(평행선·횡단선)"}
            </button>
          ))}
