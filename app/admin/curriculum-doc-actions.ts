...
export type { ProblemFormat, FigurePolicy, ProblemDifficulty } from "@/lib/problem-generation/core";
import { generateSectionProblemsCore, regenerateProblemCore, generateFigureForProblemCore } from "@/lib/problem-generation/core";

/** AI 문제 생성 — 코어는 lib/problem-generation/core.ts. 여기서는 관리자 인증만 한다. */
export async function generateSectionProblems(params: Parameters<typeof generateSectionProblemsCore>[0]) {
  await requireAdmin();
  return generateSectionProblemsCore(params);
}
export async function regenerateProblem(params: Parameters<typeof regenerateProblemCore>[0]) {
  await requireAdmin();
  return regenerateProblemCore(params);
}
export async function generateFigureForProblem(params: Parameters<typeof generateFigureForProblemCore>[0]) {
  await requireAdmin();
  return generateFigureForProblemCore(params);
}
