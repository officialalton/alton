...
const PARALLEL_TRANSVERSAL_DESC =
  "평행선·횡단선·각(표준 템플릿, 좌표 없음 — 관계만): {type:'parallel_transversal', parallel:['m','n'](위,아래 평행선 이름 2개), transversals:[{id:'k'}](1~2개), points:[{id:'A', on:['m','k']}](교점 이름, 선택), angles:[{at:['m','k'], region:'NE'|'NW'|'SE'|'SW', label:'x°'}](어느 교점·어느 사분면·라벨; 라벨은 평문 'x°','37°','(2x + 10)°'), notToScale:true}. 지문이 말하는 선·점·각 이름은 여기 반드시 있어야 하고, 지문에 없는 것을 넣지 않는다. region 은 그림 배치용 내부 값이다 — **지문에는 north/south/east/west, region, quadrant 같은 방위 표현을 절대 쓰지 말고**(쓰면 검증에서 거부된다) 예: 'In the figure, lines m and n are parallel and line k is a transversal. If the angle marked 37° ... , what is the value of x?' 처럼 시험 문제처럼 \"the angle marked (3x + 25)°\", \"the angle at A\" 로 부른다. 두 횡단선이 서로 만나 평행선과 삼각형을 이루는 그림(예: 'lines k and j meet at point E below line n')도 그릴 수 있다: transversals 를 2개 두고 그 교점을 points:[{id:'E', on:['k','j']}] 로, 교점의 각은 angles:[{at:['k','j'], region:'N'|'S'|'E'|'W', label:'x°'}](N = 평행선 쪽·삼각형 안), 만나는 자리는 crossing:{side:'below'|'above'}. 평행선 3개, 평행선에 수직인 횡단선은 이 템플릿 범위 밖이다 — 그런 문항은 만들지 않는다.";
