...
// R3 cutover: contracts는 이제 v3 스키마(household_id/child_id)를 쓰고, sendFamilyContract는
// 초대 시점에 실제 household_id/child_id를 알 수 없어 더 이상 contracts 테이블에 쓰지 않는다
// (app/admin/contracts-actions.ts의 TODO 참고) — 따라서 "contracts" 테이블 접근은 기대하지 않는다.
...
describe("sendFamilyContract", () => {
  ...
  it("부모/학생 계정을 만들고 봉투를 발송한다", async () => {
  ...
  it("DocuSign 발송이 실패하면 상담 상태를 갱신하지 않는다", async () => {
    ...
    expect(consultUpdateEqMock).not.toHaveBeenCalled();
  });
