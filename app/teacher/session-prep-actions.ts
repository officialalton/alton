...
// P2/P3(2026-09-12) — 수동 고정 버튼과 그 실행 경로는 제거했다.
//
// 고정은 수업 시작 시 한 번만 일어난다(mark_lesson_session_started →
// freeze_session_content_at_start). 화면에서 미리 고정할 수 있으면 "준비
// 저장과 고정은 다르다"는 확정 정책이 무너지고, 준비 화면이 두 벌이 되는
// 원인이기도 했다.
//
// DB 쪽 pin_session_selection() 함수는 그대로 둔다 — 시작 시점 고정이 그
// 함수를 그대로 재사용하기 때문이다. 앱에서 직접 부르는 경로만 없앴다.
