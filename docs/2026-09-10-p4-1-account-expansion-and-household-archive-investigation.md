...
- `+ 자녀 추가`는 주 보호자만 대상으로 한다. 공동 보호자 지원은 1차 범위에서
  제외하며, 이를 위한 가구 모델 변경도 만들지 않는다.
- 자녀 이메일이 기존 Auth 계정과 중복되면 발송을 시작하지 않고, 대상 입력란에
  사용자 안내를 표시한다. 실패한 온보딩 링크를 남기는 방식은 사용하지 않는다.
- 상담 신청, 부모 계정 생성, 기존 보호자에게 자녀 추가는 유입 출처일 뿐이다.
  이후 `자녀 정보 완료 → 필요한 보호자 동의 → 교사 매칭 → 체험 Smart Notes
  동의 → 체험 수업권 → 예약 → 수업`은 자녀·수강 건 단위의 공통 경로를 쓴다.
  형제자매 상태는 공유하거나 변경하지 않는다.
