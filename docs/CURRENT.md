...
- **(R5, 2026-09-02, 신규)** Vercel Preview Deployment Protection SSO가 자동화 브라우저(Playwright 등)로 로그인 이후 화면 접근을 막는다(R4 때와 동일 현상, `vercel curl`은 우회 자격증명으로 통과하지만 앱 자체 로그인 이후 화면까지는 확인 못 함). 이번 R5 마무리 세션에서는 로컬 스택의 실브라우저 E2E로 대체 검증했다(`e2e/r5-subject-enrollment-flow.spec.ts`, 3건 통과) — Preview에서 실제 눈으로 확인하려면 제품 오너가 직접 브라우저로 SSO 로그인해야 한다.
