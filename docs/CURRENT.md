...
- **(R4, 2026-09-02, 정식 오픈 전 blocker 신규 등록)** Production Stripe 웹훅 엔드포인트가 아직 등록되어 있지 않음(현재 Stripe 계정에 있는 웹훅 2개는 전부 이번 세션 UAT용 임시 Preview 주소). Preview에서 있었던 웹훅 전달 지연은 Preview 전용 Deployment Protection + 배포마다 바뀌는 URL 때문이며 Production 구조 문제가 아님(Production 도메인 `app.alton.education`은 Deployment Protection 없음, 확인됨) — 지금 등록하지 않는다. **법인 설립·Stripe Live 계정 활성화 후 오픈 전 필수 작업**으로 등록.
- **(R6, 2026-09-02, 승인·착수 대기)** 정규수업 예약/수업권 연동/Calendar·Meet/취소·재예약·지각·노쇼/AI 회의록·Smart Notes/알림·운영 전체 범위가 승인됨. 전체 스펙은 `docs/2026-09-02-r6-scope-and-approval.md`에 원문 그대로 보존 — 새 세션은 이 문서를 확정 스펙으로 삼아 곧바로 착수한다(사용자에게 범위 재확인 불필요). `sessions_v3`→`sessions`, 기존 `sessions`→`legacy_sessions` 전환이 이번 R의 구조적 핵심. Calendly/Zoom 제거는 새 흐름 E2E 통과 후에만.
