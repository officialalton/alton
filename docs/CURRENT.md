...
- **R4** — 수업권·결제 원장(entitlement ledger + payments), **완료(2026-09-01), 단 아래 blocker 1건 있음**. 스키마(`purchases`/`payment_attempts`/`entitlement_products`/`entitlement_product_versions`/`entitlement_grants`/`entitlement_ledger`), Stripe checkout+웹훅, 관리자 수업권 원장 UI(상품·공지·환불·정산·이전), 보호자 구매 UI(체크아웃·영수증·잔액), 관리자 Google 로그인(선생님 흐름과 완전 분리), 계약 활성화 재시도 UI 전부 구현·단위/E2E 테스트 통과. 실제 Stripe TEST 모드 API로 성공/거절/환불/웹훅 중복배달 검증 완료 — **단, `charge.dispute.created` 처리 시 `purchases.status`를 `'disputed'`로 갱신하려는 코드가 실제로는 조용히 실패하는 버그를 실측으로 발견함(아래 blocker 참고, 미수정)**. 상세는 실행 로그 참고.
...
- **테스트**: Vitest(유닛, 111개 파일/640건, 2026-09-01 재실행 확인) + Playwright(E2E, `e2e/`, R4 신규 3건 포함 34건 — `--workers=1` 순차 실행 기준, 기본 병렬 설정은 알려진 파일 간 레이스 있음, 아래 blocker 참고).
...
`supabase/migrations/20260923000000_r4_purchase_receipts_lesson_type.sql` (로컬 개발 DB 적용 완료). R4 마이그레이션(`20260921000000_r2_admin_google_link.sql` 관리자 Google 연결, `20260922000000_r4_purchase_and_payment.sql` 구매/결제 스키마, `20260923000000_r4_purchase_receipts_lesson_type.sql`) 포함. R3까지는 `supabase/migrations/20260916000000_r3_trial_goal_and_classification_tags.sql`(원격 = `worpsqwqgnspddnrtnvq.supabase.co`)까지 적용됨 — R4 마이그레이션의 원격 반영 여부는 별도 확인 필요(이번 세션은 로컬 DB에서만 검증).
...
- **(R4, 2026-09-01, 신규, 미수정, 실측으로 발견)** `app/api/webhooks/stripe/route.ts`의 `charge.dispute.created` 핸들러가 `purchases.status`를 `'disputed'`로 갱신하려 하지만, `purchases.status` 컬럼은 `v3_payment_attempt_status` enum(`created/processing/succeeded/failed/cancelled/reconciliation_needed`)을 쓰고 있어 `'disputed'`라는 값 자체가 이 enum에 없다 — UPDATE가 조용히 실패하고(에러 체크 없이 무시됨) `purchases.status`는 직전 값(`succeeded`)에 그대로 남는다. 실제 로컬 웹훅 라우트에 실 dispute 이벤트를 서명해 보내 재현 확인함(아래 실행 로그 참고). `20260922000000_r4_purchase_and_payment.sql`의 설계 주석은 환불/분쟁 같은 파생 상태를 `purchases`에 별도 컬럼으로 두지 않고 `refund_requests`·`entitlement_ledger`를 조인해 계산하는 것을 의도했다고 밝히고 있어(이중 상태 저장 방지), route.ts의 현재 구현이 그 설계와 어긋난다 — enum에 `disputed`를 단순 추가하는 것이 맞는 수정인지, 아니면 분쟁 상태를 별도 테이블/조인으로 옮겨야 하는지는 정책 결정이 필요해 이번 세션에서 임의로 고치지 않음. **분쟁 이벤트가 들어오면 구매가 계속 `succeeded`로 보인다는 뜻이라 결제 감사/환불 정책에 영향 있음 — 우선순위 높은 blocker.**
