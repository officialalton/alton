...
> 정상 표시, 런타임 에러 없음).
>
> **TTFB 재측정(같은 계정·같은 warm 절차, 수정 전후 비교)**: 측정
> 조건은 이전 성능 측정 라운드와 동일(`db reset --local` 직후 seed
> 데이터, `admin@alton.education` 계정, `/admin` 하드 리로드 후
> Navigation Timing API). **DB가 그 사이 재시딩되어 학생 수 등 절대
> 규모가 이전 측정과 완전히 같지는 않다는 점은 그대로 밝힌다** — 그럼에도
> 같은 페이지·같은 계정·같은 절차로 전후 비교한 값은 다음과 같다.
>
> | | 콜드(첫 히트) | 웜 #1 | 웜 #2 |
> |---|---|---|---|
...
>
> 웜 상태 기준 약 **855.7ms → 573~665ms(평균 약 619ms)로 개선** — 로컬
> Postgres는 쿼리당 왕복 지연이 매우 작아(같은 머신) 99회 쿼리를 2회로
> 줄인 것치고 개선폭이 크게 보이지 않을 수 있다는 점을 그대로 보고한다.
> 실제 네트워크 지연이 있는 운영 환경(원격 Supabase)에서는 왕복 횟수
> 감소가 훨씬 크게 체감될 것으로 예상되나, 이번 라운드는 로컬 실측치만
> 보고하고 그 이상의 추정은 하지 않는다.
>
