...
> - **② 표·수식**: 본문의 마크다운 파이프 표를 표로 렌더(`splitLearningBlocks`), 수식 KaTeX 는 기존. AI 프롬프트에 표기 규칙.
> - **③ 도형·그래프**(`20261361`): `lib/problem-figures` — FigureSpec(coordinate_plane | geometry) 검증 + 순수 SVG 렌더러. `problem_versions.figure/figure_checked`,
...
> - **표본 생성(로컬, 실제 모델 호출 10문항)**: SPR 3·기하 2·대수 1·RW 4. 결과 — 좌표평면/평행선/직각삼각형 SVG 정상 렌더, 표(파이프)·Text 1/2 ·메모 목록 정상.
>   발견·수정: ① SPR 문항이 한국어로 나옴 → "문항은 영어, 해설만 한국어" 규칙 추가 ② Rhetorical Synthesis 응답이 배열이 아니어서 실패 → 응답 모양 정규화
>   ③ 각 라벨이 선 위에 겹침 → 라벨 halo + 평행선 위 각 라벨 위로 올림. **AI 내용 오류 예**: 직각삼각형 변 라벨을 잘못 붙임(높이 8 자리에 10) —
>   그래서 '그림 확인함' 없이는 공개되지 않는다. 다음: Preview 에서 제품 오너가 문제은행 AI 생성 → 미리보기 → 확인 → 공개 UAT.
...
>
> **관리자(제품 오너 지시)**
> - 교재 라이브러리: 단원 폴더 → **대표 키워드 폴더**(Drive 구조와 동일), 보관 교재 제외, PDF/영상 아이콘·배지. `CurriculumDocListItem.primaryKeywordLabel`.
> - Drive 자료: 키워드 선택·파일별 등록/공개 UI **제거**. `과목 자료 동기화` 한 번 = 과목의 모든 키워드 폴더 → 미등록 등록 → 미공개 고정 사본 공개
>   (`syncSubjectDriveMaterialsAction`). 이미 공개는 건너뜀(원본이 바뀌어도 자동 재공개 없음 — 공개 버전 고정 규칙). **영상도 같은 길**(kind=video), 회차 구성
>   교재 담기 목록에 PDF/영상 배지로 함께 나옴(`PickableMaterial.kind`).
> - `Drive 원본 없는 교재 보관` 버튼(`archiveNonDriveDocsAction`, 과목 단위, 확인 후) — **로컬에서 non-prod 키를 못 받아(vercel env pull 은 secret 을 가림)
>   제품 오너가 Preview 에서 한 번 눌러야 함.** 삭제 아님(과거 수업 참조 유지).
>
> **버그: "수업 시작 뒤 수업 준비에서 뺀 교재가 그대로"** — 원인: 예약 시각이 아직인데 이미 `수업 시작`(final_status=live)한 수업을 시간 기준 판정이
> '수업 준비 중'으로 보여 줌. 시작 = 그 시점 구성 고정(정책)이라 이후 변경은 반영되지 않는 게 맞았지만 화면이 그렇게 말하지 않았다.
> → live 면 화면도 '진행 중'(`startedLive`). **정책 보완**: 진행 중 수업은 교사가 `지금 구성으로 이 수업에 다시 고정`을 눌러야만 반영
> (`20261356 repin_live_session_content`: 준비된 선택은 pin-lock 그대로, 매니페스트를 회차의 현재 구성으로 재작성, 학생이 사용한 항목은 지우지 않고 뒤로).
> 종료 수업 불가. 자동 반영 없음.
>
> **잔손질**: PDF `T 텍스트` 안내("페이지에서 글을 놓을 자리를 클릭하세요") + 글 상자 강조 / 문제 화면 연습장 버튼 잠금을 채점·답 저장과 분리(boardBusy).
>
> **검증**: 통합 11(채점 8 + 재고정 3: 뺀 교재 사라짐·새 교재 들어옴·사용 기록 항목 유지·비진행 거절) / 컴포넌트(Drive 패널 6, 라이브러리 4, 셸 18, 문제 21…)
> / `app/session/[id]` 스위트 db reset 직후 전부 통과. Preview UAT 미실시.
>
> **다음 작업 단위(승인됨)**: ① 과제 v3 한 갈래 통일(교사가 수업 준비 키워드·문제 풀에서 골라 발급 → 학생은 유형별 풀이 흐름 → 교사 채점 → 정답·해설 공개 →
> 레거시 `homework_items` 읽기만). 기한·알림·AI 생성 제외. ② 문제 화면 전체(여백 포함) 필기 레이어(교사·학생) — 요청 접수, 설계 중.
...
> (id `0AKnx7roQfcSaUk9PVA`, 회사 문서 Drive 와 별개), 서비스 계정 Manager. 드라이브 읽기 → 실제 쓰기
> (과목 폴더 6개 → 단원 폴더, 실패 0)까지 Drive 에서 확인. 파일 등록·공개(Drive 원본 경로)는 **미실행**.
>
> **2026-09-14 정정 — Drive 구조는 과목 → 키워드 → 파일.** 단원·회차는 폴더를 만들지 않고, 키워드는 과목 안에서
> 여러 단원·회차에 연결(N:M 유지). 단일 단원 소속 제약·전환 대상 뷰·단원 지정 UI 제거(`20261349000000`, 적용됨).
> 제품 오너가 예전 폴더를 전부 삭제 → 동기화 재확인이 "사라진 폴더 6개"를 잡아 과목 → 키워드 순으로 재생성(확인).
>
> **Drive 원본 경로 첫 공개 성공(2026-09-14, Preview `alton-3iwwgppyk` = `a28d213`)**: `SAT Reading Test 1 / Words in
> Context` 폴더의 PDF → 불러오기 → 등록(초안) → `공개 (고정 사본)` → **고정 사본 15,343KB · 99쪽**. 중간에 막힌 것:
> Vercel 서버 번들에서 pdf.js 워커가 빠져 페이지 수를 못 읽음 → `serverExternalPackages` + 추적 포함 + 워커 경로 고정
> (`a28d213`)으로 해결. 자료 규격 v1: [`2026-09-14-lesson-pdf-spec.md`](2026-09-14-lesson-pdf-spec.md)(페이지 수 제한 없음).
>
> **완료한 것**
...
>
> **사용자가 할 조작(순서대로)**
> 1. 공개된 PDF 자료를 회차 구성에 담기(관리자 기준본 또는 교사 기본 구성 `수업 준비` > 교재 목록에서 고르기) →
>    학생 회차에 `기본 구성 업데이트`로 내려 보내기.
> 2. 교사 `수업 준비` 교재 탭에서 99쪽 PDF 페이지 렌더·목차·확대·필기·저장·페이지 이동·복귀 확인(큰 파일 성능 포함).
> 3. 학생 계정 `수업 준비`·`/materials`에서 읽기 전용 열람 확인. 보호자 계정도.
> 4. 두 계정 동시 필기(교사·학생), 상대 레이어 지우개 보존 — 아직 미확인 항목.
...
> 커밋 `10d3be58101d7a240fe761602774dd1b0ea59c81`(브랜치
> `preview/m4-integration-verification`), Preview
> `https://alton-jkawwddgc-alton7.vercel.app`, 공유 non-prod `20261345000000`까지
> 적용. 전체 테스트 **2519 통과 / 1 skip**(`supabase db reset --local` 직후
> `vitest run --no-file-parallelism`). 1 skip은 AI 실호출 테스트로
> `ALTON_AI_SMOKE=1`일 때만 돈다.
>
> **확정 정책(이번에 정한 것 — 바꾸지 말 것)**
> 1. 구성은 **자동 재계산하지 않는다.** 키워드·조건·교재 공개가 바뀌면 `업데이트
>    있음`만 표시하고, 사람이 `기본 구성 업데이트`를 눌러 미리보기를 확인한 뒤
>    적용한다. 취소하면 기존 구성 유지. **단 최초 생성·최초 상속은 자동.**
> 2. 업데이트 진입점은 **하나**다. `물려받기`·`다시 구성`을 따로 만들지 않는다.
> 3. 미리보기는 실제로 적용해 보고 롤백한다(규칙이 두 벌로 갈라지지 않게).
>    미리 본 시점의 지문(`unit_composition_fingerprint`)을 적용이 들고 가고, 그
>    사이 변경이 있으면 **거절**한다(errcode `ALT02`).
> 4. 준비안은 담을 때의 버전·순서를 유지한다. **수업 시작만** 고정한다. 화면
>    진입·저장·예약 연결은 아무것도 고정하지 않는다.
> 5. 쓸 수 없는 항목이 있으면 사유와 항목을 붙여 **시작을 차단**하고 매니페스트를
>    한 줄도 쓰지 않는다(부분 고정 금지).
> 6. 수업 준비는 **화면 하나**다(관리자 기준본·교사 기본·학생별·예약된 수업 전부
>    `/lesson-prep/[layer]/[unitId]`). 준비 중 다른 화면으로 건너뛰는 버튼을 두지
>    않는다. `수업 시작`만 별도 행동이다.
> 7. 학생·보호자는 수업 전 교재·문제를 열람한다. **정답·해설과 teaching_tip은 응답
>    데이터에 담기지 않는다**(`unit_preview_for_viewer`가 담을 것만 담는다 — 화면에서
>    가리는 것이 아니다). 답안 제출·채점은 범위 밖.
> 8. 문제은행 조회는 **담당 과목**(`teacher_curriculum_templates`) 기준이다.
> 9. 문제가 모자라도 AI로 자동 충원하지 않는다 — 숫자로만 알린다.
>
> **이번에 드러나 고친 결함 5건**
> - `seed_curriculum_overlay_for_match`가 `source_teacher_template_unit_id`만 적고
>   `source_unit_id`를 비워 둬서, **교사 기본 구성이 있는 정상 매칭일수록** 학생
>   회차가 텅 빈 채로 만들어졌다. 상속 트리거가 후자만 봤기 때문이다. 화면의 `보강`
>   배지도 같은 원인(판정이 `source_unit_id is null`). 제약
>   (`curriculum_overlay_units_source_consistency`)이 두 칸 동시 사용을 막으므로,
>   상속·물려받기·배지가 교사 회차 참조만으로도 동작하도록 맞췄다.
> - `problems` 조회 정책에 문제은행 분기가 없었다(`section_id`·`origin_session_id`가
>   둘 다 null). 선생님 수업 준비의 문제 후보가 **항상 비어 있었다.**
> - 버전 스탬프·소속 확인 트리거가 호출자 권한으로 돌아, 선생님이 담은 교재에 버전이
>   거의 기록되지 않았다(`prep_version_coverage`의 `no_reference`). security definer로
>   바꿨다. **과거 no_reference 13건이 전부 같은 원인이라고 단정하지 않는다** — 생성
>   시점·경로 기록이 없다.
> - `session_problem_attempts.session_id`가 R6 cutover 이후 `legacy_sessions`를
>   가리켜, **수업 중 문제를 풀면 FK 위반으로 실패**했다. v3 `sessions`로 옮겼다
>   (`20261337000000`, 옮기지 못한 값은 `session_problem_attempt_legacy_links`에 보존).
> - `pin_session_selection`이 준비안의 버전이 아니라 시작 시점의 최신 공개본을 집었다.
>
> **다음 작업 단위 — 상위 변경의 완전한 반영**
> 지금 `기본 구성 업데이트`는 **보충과 버전 갱신만** 한다. 미구현:
> 상위에서 **제외된 항목**을 아래 계층에서도 빼기 / **키워드·목표·순서** 변경 반영 /
> 교사가 직접 담은 것·뺀 것과의 **충돌 구분**. 막힌 이유는 각 구성 행에 "상위에서
> 내려온 것인가"라는 표식이 없어서다 — 지금 빼면 교사가 고른 것을 지울 수 있다.
> provenance(예: `inherited boolean`)와 목표의 상속 기준값을 먼저 설계한다.
>
> **열린 결함**
> - 테스트 격리: 전체 실행이 드물게 `reservations_no_overlap`로 깨진다
>   (`auto-link-next-unit` 등). 단독 실행은 통과. **공유 non-prod 데이터를 정리해
>   덮지 않는다** — 별도 추적 대상이다.
> - `homework_items.session_id`도 `legacy_sessions`를 가리킨다(`20261235000000`이
>   확인하고 우회해 둔 상태). homework 간헐 실패와 같은 계열일 가능성은 있으나
>   **단정하지 않았다.**
> - Google 이전 배포 실패 원인, Drive 비관리자 차단(teacher1로
>   `company-documents-preflight`) 미확인.
>
> **AI 생성** — Preview에 `ANTHROPIC_API_KEY` 설정됨(Secret, Preview만). 모델
> `claude-sonnet-5` 호출 가능 확인. 실호출 성공은 **로컬 1회**뿐이고 **Preview
> 화면에서의 생성 → 초안 저장 → 다시 열기는 미확인**이다. 반복 호출 최소화.
>
> **제품 오너 Preview 확인 대기**: ① 새 학생 배정 후 버튼 없이 수업 준비에
> 키워드·교재·문제가 있는지(`보강` 배지 사라졌는지) ② `기본 구성 업데이트`
> 미리보기 → 적용 ③ 예약된 수업 → 수업 준비가 같은 화면이고 `이 수업에 연결` /
> `수업 시작`이 있는지 ④ 학생 계정의 `교재·문제 미리보기`(정답·해설 비노출)
> ⑤ 문제은행 `AI로 만들기`.

> **2026-09-14 — 제품 오너 백로그 추가(문서만, 구현 상태 변경 없음).**
> [제품 백로그](2026-09-14-product-backlog.md): ① 학생·교사·보호자의 과목별 전체 교재 보기 — 기존 키워드 연결·라이브러리·뷰어를 재사용해 현재 P2/P3와 연계 반영 우선. ② 마스터/중간 관리자 업무별 권한 배분 — 낮은 우선순위 후속. 기존 관리자 정책은 유지한다.
...
> **Production 무변경, `real_disbursement_enabled()` = false 유지.**
> **실제 결함 2건 수정**: ① 도입 전 승인 건의 지급 예정일이 비어 있어 자동 송금
> 대상 조회에서 **영원히 빠지던** 문제 — 각 묶음의 **실제 승인 시각** 기준으로
> 보정(지금 시각이 아니라). ② 감사 로그 처리자가 전부 "알 수 없음"이던 문제 —
> 이름 조회 맵이 **교사 id로만** 만들어져 관리자 id가 없었다.
> **자동 송금 기본값 — 두 정책 분리(테스트로 고정)**: 새로 승인되는 묶음은
> **대상 포함**(`approve_payout_batch`가 설정), **도입 전 승인 건 보정은 대상
> 제외**(자동 송금 판단을 한 적이 없고, 일괄로 켜면 Wise 게이트를 여는 순간 과거
> 건이 한꺼번에 나간다), 제외해둔 묶음의 **재승인은 다시 포함**.
> **외부 송금 완료 기록**: 확인 메모를 **선택값**으로 정정(거래번호는 송금 실행에
> 필요한 값이 아니라 사후 대사 보조 정보). 필수는 **송금 완료일 + 승인된 최종
> 송금액 일치**뿐이고 처리자·시각은 시스템이 기록한다. 필수값 누락 시 각 입력칸
> 아래 즉시 오류, 금액은 읽기 전용.
> **관리자 화면**: Wise 게이트가 닫혀 있으면 `지금 송금 요청`을 실행 버튼 대신
> 비활성 안내로, `지급 실패 기록`은 송금 요청 이후에만 노출(현재는 숨김), 승인
> 묶음 작업은 기본 접힘, 항목·감사 로그 한국어화 + 조정 사유·실제 처리자 표시.
> **검증**: `db reset` 후 직렬 전체 **284 files / 2049 tests 통과**(295.8s).
...
> 마이그레이션 2건(`20261286000000`, `20261287000000`)을 공유 non-prod에 적용
> 완료(pending 0). **Production 무변경.** 상세는
> `docs/2026-09-12-p4-2-teacher-settlement-plan.md` "3차/4차" 절.
> **교사 포털**: 지급 예정일 **익월 10일** 표기 + "매월 10일에 전월 수업분 지급"
> 안내, 상태별 지급 일정 문구(검토 중은 "검토 완료 후 지급 (예정일 …)"),
> **정산 현황/정산 내역/계좌/서류 4개 서브탭**, 계좌 폼 라벨화 + 통화 KRW/USD
> 선택 + "띄어쓰기 없이 하이픈 넣어서 작성" 안내, 금액 **₩/$ 기호** 표시,
> 업로드 버튼화, **서류 삭제** 추가.
> **상태 매핑 변경**: 묶음이 만들어진 것만으로는 '검토 중'이 아니다 —
> `draft`/`calculated`는 계속 **예정**, 관리자가 `검토 제출`을 눌러 `reviewing`이
> 된 뒤부터 **검토 중**.
> **관리자 포털**: 목록이 사라지던 **실제 버그 수정**(SSR initialBatches를
> useState 초기값으로만 쓰다가 클라이언트 탭 전환 시 빈 배열로 굳음 → 마운트 시
> 직접 조회), **`월 마감 실행` 버튼 노출**(자동 마감과 같은 경로, 기존 Batch
> 생성은 '구경로'로 개명), **승인 전 묶음 삭제** 추가.
> **묶음 삭제 정책(제품 오너 판단으로 변경)**: 조정이 붙어 있어도 승인 전이면
> 삭제할 수 있다. 대신 `payout_batch_adjustments`는 평시 UPDATE·DELETE 모두
> 금지인 채로 두고 `delete_payout_batch()`가 트랜잭션 안에서 세우는 플래그가
> 있을 때만 DELETE가 열린다. 관리자 조정은 묶음과 함께 사라지고, 마감 뒤
> 재판정으로 생긴 **이월 조정은 살아남아** 미배치로 돌아간다. 승인 이후 묶음은
> 어떤 경로로도 삭제 불가.
> **확인된 사실 — 지급 일정은 아직 "표시"일 뿐이다**: `payout_batches`에 지급
> 예정일 컬럼이 없고, 승인은 아무것도 촉발하지 않는다(`approved` + `approved_at`
> 기록이 전부). 송금 체인(`dispatch_payout_batch` → `provider_pending` → `paid`)은
> DB에 있으나 전부 `real_disbursement_enabled()`(현재 false) 뒤에 있고 **앱에
> 호출 경로가 없다**. 재시도 안전장치(배치 `dispatch_idempotency_key` unique,
> 항목 `provider_transaction_id`/`retry_count`)는 이미 준비돼 있다.
> **금융 연동 전 남은 결정 3건**: ① 10일에 누가 dispatch를 부르나(관리자 버튼 vs
> 10일 크론) ② 승인이 10일을 넘겼을 때 규칙 ③ 지급 예정일을 데이터로 남길지
> (`scheduled_payout_date` 컬럼 — 계산값으로 두면 규칙 변경 시 과거 표시도 바뀐다).
> **검증**: `db reset` 후 직렬 전체 **282 files / 2002 tests 통과**(310.3s).
...
> `upsert_session_payout_item()`이 세션 최종 판정마다 `payout_items` 1행을
> 만들고 갱신하며, `generate_payout_batches()`가 **예약 시작일 기준 월**로 묶는다.
> `payout_items`/`payout_batches` RLS 조회 정책이 `teacher_id = auth.uid()`를
> 이미 허용해 **교사 정산 화면에 새 RLS·새 RPC가 필요 없었다.** 없던 것은 교사
> 수취 계좌·제출 서류 테이블, Storage 버킷, 교사 포털 진입점뿐이다. 레거시
> `payouts` 테이블(`payouts-data.ts`)은 쓰지 않고 v3 payout_batches만 원본으로 썼다.
> 1. **교사 포털 `정산` 탭 신설**(홈/담당 학생/수업/가능시간/커리큘럼/교재 다음).
>    다음 지급 예정액(통화별) + 지급 예정 월 + 마지막 갱신 시각 + "확정 전까지
>    변동 가능"·"공제 미반영 총액" 안내, 월별 내역(수업 월/지급 예정 월/상태/금액),
>    월을 펼치면 수업별 산출 근거(날짜·학생·과목·지급 대상 분·시급 스냅샷·금액).
> 2. **정산 상태 4단계(2026-09-12 제품 오너 확정 흐름으로 교체)**:
>    `예정`(묶음 없음 — 최종 판정된 수업의 실시간 합계, 변동 가능) →
>    `검토 중`(월 마감 후 묶음 생성, 관리자 확인: `draft`/`calculated`/`reviewing`/
>    `reviewed`/`failed`) → `송금 승인됨`(운영자의 **유일한 승인 지점**:
>    `approved` 이후 `dispatch_requested`/`provider_pending`/`processing`까지
>    실행 단계로 함께 묶어 표시) → `지급 완료`(`paid`). 금액은
>    `payout_items.amount_minor`를 그대로 쓰고 **사람이 수기로 계산·확정하는
>    경로를 만들지 않는다.** 마감 뒤 변동은 승인된 금액을 덮어쓰지 않고 다음
>    정산월 조정 항목으로 반영한다(DB도 `prevent_paid_item_mutation`/
>    `reverse_payout_item()`로 같은 방향). **`paid` 전이·실제 송금·금융사 연동은
>    범위 밖이며 화면은 읽기만 한다.**
>    *아직 없는 것(금융 연동 단계에서 붙임)*: 월말 묶음 **자동 생성**(현재는
>    관리자가 기간을 넣어 수동 실행), 다음 정산월 조정 항목 **자동 생성**.
> 3. **수취 계좌**: 교사 본인이 등록·수정(`teacher_payout_accounts`, 교사당 1행).
>    **전체 계좌번호는 어떤 응답에도 들어가지 않는다** — 교사 본인 화면도 `****1234`만
>    받고 수정은 전체 재입력이다. 변경 이력(`teacher_payout_account_events`,
>    INSERT-only)에도 전체 번호를 복제하지 않고 바뀐 필드 이름과 끝 4자리 전/후만 남긴다.
> 4. **관리자 `정산` > `수취 계좌` 서브탭**: 교사와 **같은 원본·같은 마스킹 함수**
>    (`maskAccountNumber`)를 쓰고 변경 이력을 펼쳐 본다. 관리자는 조회 전용이다.
> 5. **교사 제출 서류 = 업로드·보관 창구**: 비공개 Storage 버킷 `teacher-documents`
>    (경로 `<teacher_id>/...`) + `teacher_documents` 메타. **승인·검토·보완 상태
>    컬럼을 아예 두지 않아** 나중에도 게이트로 오용될 수 없다. 화면에도 "필수/미제출"
>    표현을 쓰지 않는다. P4-3 관리자 문서 탭이 이 테이블·버킷을 읽어 교사별 조회·
>    다운로드를 붙이면 된다.
> **범위 밖(고정)**: 실제 송금, `paid` 전이, Mercury/Wise·Stripe Live, Drive 권한·
> 환경변수 변경. 지급 게이트(`real_disbursement_enabled()`)는 손대지 않았다.
> **미확정(제품 오너 확인 필요 — 아래 가정으로 구현)**: ① 예정액은 최종 판정이
> 끝나 `payout_items`가 생긴 수업만 포함(예약만 된 미진행 수업 제외), ② 지급
> 주기는 "수업 월 기준 익월"만 표시하고 구체 마감일·지급일은 표시하지 않음,
> ③ 세금·수수료·보류 **공제 미반영 총액**을 표시, ④ 제출 서류 종류 목록이 없어
> 자유 업로드로 두었다.

> **2026-09-11 — 교사 "학생 운영 커리큘럼"(v3) 지연 제보: 현재 재현 안 됨,
> 원인 미확정으로 종결(추가 리팩터링 보류).** 대상 화면: 교사 포털 "커리큘럼"
> → "학생별" → 학생·과목 클릭 → `StudentCurriculumOperatingView`
> (`app/teacher/CurriculumTab.tsx`) → `loadStudentCurriculumPanelData()`
> (`app/teacher/student-curriculum-actions.ts`, v3
> `subject_enrollments`/`teacher_assignments`/`curriculum_overlay_units`
> 경로 — 같은 날 고친 `legacy-curriculum-actions.ts`는 레거시 `enrollments`
> 테이블 경로라 이 화면과 무관, 착각하지 말 것). 확인된 실제 대상: 교사
> `teacher1@alton.education`("Teacher test1"), 학생 "테스트 자녀 9-1",
> 과목수강 `baba3a3b-af69-44ef-834c-6f183e770191`(과목 "테스트1 444"),
> 배정 `8e1ef1fb-383b-43f1-83b3-846f61862ee8`. 제품 오너가 이 정확한
> 계정·학생·과목으로 배포 커밋 `897b717`(Preview
> `https://alton-odtk3yron-alton7.vercel.app`)에서 "테스트 자녀 4"·
> "테스트 자녀 9-1" 둘 다 직접 재확인해 "현재 정상 속도"로 확정했다 —
> 최초 보고된 10초 이상 지연은 이 세션에서는 재현되지 않았다.
> **원인 미확정.** 같은 날 다른 화면(레거시 커리큘럼, 담당 학생 50명 벌크
> 계정)에서 실제로 DB 커넥션 경합·Gateway Timeout까지 확인된 전례가 있어
> (아래 항목 참고) 유사한 일시적 부하 요인일 가능성이 있으나, 이 v3
> 경로 자체를 그 시점에 직접 측정하지는 못했다(로그 스트리밍
> `vercel logs --follow`가 이 세션에서는 실제 요청을 잡지 못함 — 원인
> 미확인, 별도 도구 이슈일 가능성).
> **다음에 다시 느려지면**: 같은 계정(`teacher1@alton.education`)·같은
> 학생/과목·같은 진입 경로(커리큘럼 → 학생별 → 클릭)로, 로그인 직후
> 캐시 없는 첫 클릭부터 측정한다. 브라우저 Performance/Network 탭의
> Server Action POST 응답 시간과, 그 시각 `vercel logs`(가능하면
> `--follow` 대신 사후 조회로 재시도)를 함께 대조해 "클릭→요청 시작·
> 요청 전송·서버 처리·응답 렌더" 중 실제로 느린 구간을 먼저 특정한 뒤에만
> 수정한다 — 서버 함수 내부 시간만으로 원인을 단정하지 않는다(이번
> 세션에서 반복 지적된 방법론).

> **2026-09-11 — P4-1(B) 경량 가구 아카이브·복귀: 완료(제품 오너 Preview UAT 통과·완료 승인).**
...
>    버튼 + 영향 미리보기 확인 모달.
> 7. **목록 제외 범위(확정 범위 전체)**: 학부모(`loadParents`)·학생(`loadStudents`)·
>    매칭 대기(`loadStudentsForMatching`)에 더해 **신규 보드(`loadKanbanBoard`)·
>    계정 생성 카드(`loadAccountCreationCards`)·발송 내역
>    (`listDirectOnboardingLinksAction`)** 에서도 제외한다. 보드는 상담 카드와 계정
>    생성 카드가 모두 `child_id`(계정 생성 카드는 `child_auth_user_id`)를 쓰므로
>    합친 뒤 한 번만 거른다. 발송 내역은 링크 1건이 보호자 1명(=가구 1개)에
>    귀속되므로 사용 보호자(`redeemed_auth_user_id`) 또는 만들어진 자녀가 아카이브
>    가구면 링크 전체를 숨긴다. 목록당 왕복 +1.
> **범위 밖(기록)**: 상담 예약(`consult_requests`)은 이메일 기반이라 가구에 귀속되지
> 않아 이번 아카이브 대상에 넣지 않았다. 공동 보호자 가구 모델 변경도 하지 않았다.
> **검증**: DB 통합 8 + 오케스트레이션 유닛 7 + 컴포넌트 6 + 목록 필터 3 +
> 보드·발송 내역 제외 6 신규 통과(직렬 전체 273 files / 1919 tests 통과, 301.1s),
...
> **마이그레이션 반영 완료**: `20261283000000`을 공유 non-prod
> (`worpsqwqgnspddnrtnvq`, 제품 오너 승인 2026-09-11)에 적용했다 — dry-run으로
> 적용 대기가 이 파일 1건뿐임을 확인한 뒤 push, 적용 후 pending 0 확인.
> **Production은 무변경.**
> **Preview UAT**: 실제 가구에서 아카이브·복귀를 확인했고, 복귀 뒤 종료된 매칭과
> 취소된 예약이 자동 복원되지 않음을 제품 오너가 확인했다(UAT 실행 ID
> `p4-1b-archive-20260911`). 진행 중(`live`) 수업이 있는 가구의 차단은 DB·오케스트레이션
> 자동 검증으로 기준선을 유지하며, 별도의 UAT 시나리오를 강제하지 않는다.
> **기준선 고정(2026-09-12 제품 오너 지시)**: P4-1 A/B 모두 **완료 기준선**이다.
> 이후 라운드는 이 동작(관리자 링크 발송 기반 자녀 추가, 가구 아카이브·복귀)을
> 그대로 기준으로 삼고 다시 설계하지 않는다.
> **다음**: P4-2(교사 정산 흐름) 착수 — 아래 항목 참고. P4-3(관리자 문서 탭)은
> 계획 문서로만 유지한다. 서류 제출은 업무 게이트가 아니라 업로드·보관 창구다
> (`docs/2026-09-10-p4-3-admin-documents-tab-design.md:19-20`에 이미 반영).

> **2026-09-11 — P4-1(A) 기존 주 보호자에게 자녀 추가: 완료(제품 오너 UAT 통과·
> 완료 승인).** 커밋 `d158cc0`(`preview/m4-integration-verification`), Preview
> `https://alton-6krtt3nv8-alton7.vercel.app`. 착수 1장 정리는
> `docs/2026-09-11-p4-1-add-child-to-guardian-plan.md`.
> **현재 흐름(관리자가 온보딩 링크를 발송하고 보호자가 확인해 자녀 계정이 생성되는
> 경로)을 그대로 유지한다 — 라운드 중 논의됐던 "즉시 연결" 방식으로 바꾸지 않는다.**
...
> **검증 기준선(2026-09-11 확정)**: 전체 스위트 합격 판정은
> `npx supabase db reset --local && npx vitest run --no-file-parallelism` 결과로
> 한다. 병렬 기본 실행의 무작위 실패는 합격·불합격 판정에 쓰지 않는다.
> **후속 과제(별도 배치, 이번 범위 아님)**: 통합 테스트의 공유 DB 병렬 간섭 개선
> (고정 라벨 픽스처 → 실행별 고유 값, 전역 테스트 트리거의 파일 간 격리).
> **UAT**: 제품 오너가 Preview에서 `+ 자녀 추가` 실제 발송 → 보호자 링크 확인 →
> 자녀 계정 생성까지 확인하고 완료 승인했다(2026-09-11). 남은 작업 없음.
> P4-3(관리자 문서 탭)은 이번 라운드 미착수.
...
> 재매칭 건 → 정규 계약 대기" 일괄 판정)을 반려 — 종료 이력은 계약 상태나
> 체험권 사용 여부의 신뢰할 수 있는 대리 지표가 아니라는 지적에 따라, 기존
> 계약·체험권·수업권 판정 경로만 근거로 재구현했다.
> 1. **체험 자격 판정을 실제 체험수업권 잔량 기준으로 변경**: 'planned'
>    상태라고 무조건 체험 후보로 보여주던 것을, 이제 `entitlement_grants`/
>    `entitlement_ledger`에서 이 학생(child 단위 — 체험수업권은 상담 1건당
>    1개, M2 정책)의 미만료 체험수업권 실제 잔량을 계산해 판정한다. 잔량이
>    있으면(또는 이 수강 건에 이미 hold된 미소진 예약이 있으면 — "이미
>    예약함" 안내를 그대로 보여주기 위해) 체험 후보 유지. 재매칭 건이라도
>    체험수업권이 남아있으면 그대로 체험 후보다(종료 이력만으로 미사용
>    체험 자격을 없애지 않는다) — 반대로 체험이 이미 소진됐으면(재매칭
>    여부와 무관하게) 후보에서 빠진다.
> 2. **체험 후보가 아닐 때의 실제 차단 사유 3단계 구분**: 기존 활성화
>    판정 경로(`subject_enrollment_activation_ready` RPC, R5/M4가 이미
>    쓰는 유일한 경로 — 2026-09-05부로 "기본계약 active"만 확인하도록
>    단순화됨)와 실제 정규 수업권 잔량을 그대로 재사용한다: 계약이 아직
>    active가 아니면 `contract_pending`("정규 계약 대기"), 계약은 active인데
>    정규 수업권이 없으면 `no_entitlement`("사용 가능한 수업권 없음"),
>    계약도 active고 정규 수업권도 있으면 `activation_pending`("계약·
>    수업권 모두 확인됨 — 활성화 처리 대기", 수업권 없음으로 잘못 안내하지
>    않음 — 정규 예약 조건 자체는 이미 충족됐다는 뜻). 상태를 추정하지
>    않고 실제 확인된 값만 사용.
> 3. **완료된 수업은 리뷰 여부와 무관하게 항상 "지난 수업"**: `isPastSession()`
>    이 체험 수업이면 리뷰 확정 전까지 최종판정과 무관하게 "예정 수업"에
>    남겨두던 로직을 제거 — 완료(finalized)되면 무조건 지난 수업으로
>    분류하고, 리뷰가 아직 필요하면(체험 + completed + 리뷰 미확정) 별도
>    `needsReview` 플래그로 "리뷰 작성 필요" 배지만 표시한다("예정 수업"·
>    "수업 시작" 대상에서는 확실히 제거).
> **테스트 학생 7-1의 실제 차단 사유**: 코드 추적으로 확정한 근본 원인은
> 여전히 (a) `hasExistingTrialBooking`의 과목명·선생님명 문자열 매칭(4차
> 1차 수정에서 이미 subjectEnrollmentId 비교로 교체 완료)과 (b) 재매칭
> 건의 체험/정규 판정 로직(이번 배치에서 실제 잔량·계약 상태 기준으로
> 교체) 두 가지였다 — 실제 데이터(체험 사용 이력·계약·수업권 상태)는
> Preview에서 제품 오너가 최종 확인.
> **검증**: 재매칭 시나리오 5종(체험 미사용 후 재매칭·체험 소진+계약대기·
> 계약active+수업권없음·계약active+수업권있음·이미 hold된 체험 유지)과
> 리뷰 무관 "지난 수업" 분류 4종을 포함한 신규/변경 유닛 테스트, tsc/eslint
> 클린(무관 pre-existing 오류 4건 동일 확인), `supabase db reset --local`
> 후 전체 263 files/1853 tests 통과, `next build` 성공. **다음**: 제품
> 오너가 Preview에서 체험 전 매칭 종료 후 재매칭 / 체험 사용 후 재매칭 /
> 정규 계약·수업권 보유 후 재매칭 세 시나리오를 실제 계정으로 검증 —
> 통과하면 C-2 종료.
>
> **2026-09-11 — C-2(4차): 매칭 종료·재매칭 UAT 후속 결함 2건 수정(migration
> 없음, app 코드만).** 제품 오너가 C-2(3차)에서 매칭 종료·동일 교사 재매칭
> 정상 확인 후 발견.
> 1. **종료된 수강의 담당 교사 표시**: 학생·학부모 "수강 과목"에서 종료된
>    건이 활성 매칭이 없다는 이유로 "담당 선생님: 배정 전"으로 표시되던
>    것을 수정 — 이 수강 건 자체의 종료 이력(`e.history`, enrollment
>    단위로 이미 스코프돼 다른 수강 건의 교사가 섞이지 않음)에서 마지막
>    교사를 찾아 "마지막 담당 선생님: 이름"으로 보여준다. 정말 배정된 적
>    없는 경우(이력 자체가 없음)만 "배정 전" 유지.
> 2. **재매칭 후 체험 예약 오차단**: 대상(테스트 학생 7-1/테스트1 444/
>    Teacher test1)을 코드로 추적한 결과 두 가지 실제 결함을 확인.
>    - `hasExistingTrialBooking`(체험 1회 제한 판정)이 과목명·선생님명
>      문자열 일치로 판정해, 매칭 종료된 옛 수강 건의 예약이 같은 과목·
>      선생님으로 재매칭한 새 수강 건까지 "이미 체험 예약함"으로 잘못
>      막았다 — `subjectEnrollmentId`로만 비교하도록 수정
>      (`UpcomingBooking.subjectEnrollmentId` 필드 추가).
>    - `loadLessonBookingData()`가 `subject_enrollments.status`만 보고
>      'planned'면 무조건 "체험" 후보로 취급해(`isTrial = status !==
>      'active'`), 같은 과목에 이미 종료된 수강 이력이 있는 재매칭
>      건(정규 계약이 있었을 수 있음)에도 새 체험 슬롯을 내줄 뻔했다
>      (재매칭만으로 체험 기회가 새로 생기면 안 된다는 정책 위반). 같은
>      과목에 종료·완료 이력이 있는 'planned' 건은 체험 후보 목록에서
>      제외하고 "정규 계약 대기 상태입니다" 안내로 분리(신규
>      `pendingActivationSubjects`) — 캘린더를 띄우기 전에 바로 안내.
>      제품 오너 요청대로 가장 낮은 난이도로 구현(신규 조회·게이트 없이
>      이미 로드된 수강 이력만 재사용).
>    - 부수 발견·수정: `isPastSession()`이 체험 수업이면 최종판정 값과
>      무관하게 무조건 리뷰 확정 전까지 "예정 수업"에 남겨뒀다 — 취소·
>      노쇼 등 리뷰가 필요 없는 최종판정까지 계속 "예정 수업"·"수업 시작"
>      대상으로 잘못 노출될 수 있었다. `completed`(리뷰 필요)일 때만
>      리뷰 확정을 기다리도록 좁혔다.
> **검증**: 신규/변경 유닛·컴포넌트 테스트(재매칭 후 체험 후보 제외·정규
> 계약 대기 분류, 다른 수강 건 예약이 새 체험을 막지 않음, 리뷰 불필요한
> 최종판정은 즉시 지난 수업으로 등 포함), tsc/eslint 클린(무관 pre-existing
> 오류 5건 `git stash`로 재확인 — 이번 변경 전과 동일 개수), `supabase db
> reset --local` 후 전체 263 files/1851 tests 통과, `next build` 성공.
> **다음**: 제품 오너가 Preview에서 재현 확인 — 학생/학부모 "수강 과목"의
> 종료된 과목이 "마지막 담당 선생님"으로 보이는지, 테스트 학생 7-1이
> 이제 예약 캘린더까지 정상 진입하는지(계약·수업권이 이미 있다면) 또는
> "정규 계약 대기"/수업권 부족 안내를 받는지.
>
> **2026-09-11 — C-2(3차): 매칭 종료 실패 결함 수정 + "매칭" 용어 통일 +
> 제품 정책 확정(수강 종료를 별도 기능으로 두지 않음).** 제품 오너가 Preview
> UAT 중 특정 매칭 종료가 "이미 consume된 예약은 release할 수 없습니다"로
> 실패하는 걸 발견.
...
> 예약"을 판정했는데, `finalize_lesson_session()`(수업을 미리 시작→종료
> 판정)이 실행돼도 `reservations.status`는 바뀌지 않는다(그 함수는
> `sessions.final_status`만 바꿈) — 그래서 실제로는 이미 수업이 진행돼
> 수업권이 consume된 세션의 예약도 "아직 안 정리된 미래 예약"으로 잘못
> 잡혀 종료 처리가 `release_entitlement()`를 시도하다 실패했다.
> **정책(제품 오너 확정)**: 완료된 수업과 이미 쓴 수업권은 보존 — 취소·
> release로 되돌리지 않는다. 진행 중(live)인 수업이 있으면 종료 자체를
> 보류하고 안내한다. "수강 종료"와 "매칭 종료"는 별도 사용자 기능으로
> 나누지 않고 "매칭 종료"로 통일 — 매칭 종료 시 활성 매칭과 수강 건을
> 함께 종료하고, 취소 가능한 미래 예약은 기존 취소 정책대로 처리, 지난
> 수업·커리큘럼·계약·결제 이력은 보존하며 계약 해지·환불은 자동 실행하지
> 않는다. 이후 같은 과목을 같은/다른 교사와 새로 매칭할 수 있다(과거
> 종료 이력·취소된 예약은 자동 복원 안 함).
> **조치**: (1) migration `20261277000000` — 두 함수 모두 `sessions.
> final_status in ('scheduled','live')`인 예약만 "처리 대상"으로 좁힘(그
> 외는 이미 최종 판정 끝난 이력이라 종료 처리 대상에서 제외). 감사 이력
> `action` 허용값에 `skipped_already_delivered` 추가. (2)
> `processTeacherAssignmentTermination()` — 진행 중(live) 세션이 하나라도
> 있으면 어떤 예약도 건드리기 전에 즉시 실패(부분 진행 방지, 안전한
> 재시도 보장) + 이미 최종 판정된 세션의 예약은 감사 이력만 남기고
> 취소를 시도하지 않음. (3) 관리자 화면(영향 확인 화면·요청 처리 화면)의
> "영향받는 예약" 표시를 "취소될 예정/이미 완료·종결/진행 중" 세 범주로
> 나눠 실제 처리 대상과 화면 표시를 일치시킴, live 세션이 있으면 실행
> 버튼도 비활성화.
> **용어 통일**: 관리자 화면 전체에서 새 배정→새 매칭, 배정 확인→매칭
> 확인, 배정 종료→매칭 종료, 재배정→재매칭, 배정된/미배정→매칭된/
> 미매칭으로 통일(선생님 변경은 별도 흐름으로 그대로 유지, 내부 함수명·
> DB 이름은 미변경). `subject_enrollments.status`를 화면에 raw enum으로
> 노출하던 것도 한국어 라벨로 교체. 성공 메시지가 항상 빨간 오류색으로
> 표시되던 버그도 함께 수정(성공/오류 색 구분).
> **검증**: 신규 DB 레벨 e2e(`e2e/c2-termination-preserves-delivered-
> sessions.spec.ts`, 완료/취소 등 최종 판정 상태별 게이트 통과·거부 5건),
> 신규/변경 유닛 테스트(진행 중 수업 차단, 이미 완료된 세션 건너뛰기 등),
> tsc/eslint 클린, `supabase db reset --local` 후 전체 263 files/1845
> tests 통과, `next build` 성공, `playwright test`(R5 12건 + 신규 5건)
> 17/17 통과. **다음**: 제품 오너가 Preview에서 실제로 실패했던 케이스
> (테스트 학생 7-1/테스트1 444/Teacher test1) 재현 확인.
>
...
> 자녀에게 Smart Notes 동의가 강제되던 문제 해소 확인. **미검증 항목(문서화,
> Production 전 별도 검증 필요)**: 정규 계약의 실제 이메일 발송·수신 —
> Preview 환경은 안전장치로 실제 DocuSign 발송을 의도적으로 차단하므로
> (`app/admin/ConsultationKanbanBoard.tsx:715-722`, `DOCUSIGN_SANDBOX_
> ALLOW_REAL_CALLS` 플래그, 계약은 draft로 남고 "발송 실패 — 관리자 조치
> 필요" 안내가 뜨는 것 자체가 정상 동작) 이번 UAT에서 검증한 것은 계약
> 생성·발송 대기 상태·계약 대상자(보호자·학생) 연결·"신규 > 정규 계약
> 발송" 화면 노출까지이며, **실제 이메일 발송·수신 확인은 Production
> 배포 전 별도 체크리스트 항목으로 남긴다.**
>
...
> 추가하는 경로가 없다. 요청 사항: 기존 "+ 계정 생성" 버튼을 **"+ 부모
> 계정 생성"**으로 이름을 바꾸고, 그 아래 **"+ 자녀 추가"** 버튼을 새로
> 추가한다 — 기존 보호자를 선택해 그 보호자에게 자녀 계정만 추가하는
> 흐름(새 보호자 생성 없이 `households`/`household_members`에 자녀만
> 연결). 별도 배치로 설계·구현 예정, 이번 라운드에는 포함하지 않음.
>
> **2026-09-10 — P0(4차): 초대 링크가 잔여 쿠키 때문에 /login으로 튕기는
> 결함 + 신규 현황 보드 계정 생성 카드 자녀 구분 표시.** 제품 오너가 새
> 계정 생성 후 비밀번호 설정 링크를 눌렀는데 그냥 `/login`으로 이동하는
> 것을 발견(재현 확인 — 스테일 토큰/구버전 배포 가설은 오류 파라미터가
> 없다는 점으로 배제). **원인**: `/api/trial-onboarding/redeem`가 "이미
> 로그인된 사용자인지"를 `cookieHeader.includes("sb-")`로만 판정했는데,
> Supabase가 로그아웃 후·PKCE 시작 시에도 남기는 잔여 `sb-` 쿠키 때문에
> 세션이 없어도 로그인된 것으로 오판 → `requireUser()`가 걸린
> `/consult/trial-onboarding`으로 잘못 보내지고 세션이 없으니 곧바로
> `/login`으로 튕겼다(관리자가 같은 브라우저로 신규 초대를 테스트하는
> 패턴이라 100% 재현). **수정**: 쿠키 문자열 대신 실제
> 세션(`auth.getUser()`)으로 판정. 회귀 테스트 4건 추가. 같은 라운드에서
> 신규 현황 보드의 "계정 생성" 카드가 한 보호자당 자녀 여러 명이면 전부
> 같은 보호자 이름·이메일만 보여 구분이 안 되던 것도 함께 수정 —
> `requested_children`(다자녀 상담 카드가 이미 "자녀 N명: 이름"으로
> 렌더링하던 필드)을 재사용해 새 UI 코드 없이 자녀 이름을 표시.
> **검증**: `tsc`/eslint 클린, `supabase db reset --local` 후 전체 스위트
> 260 files/1813 tests 통과, `next build` 성공. **외부 변경**: 커밋
> `905125e` push, Preview 배포 →
> `https://alton-6u5afkwcq-alton7.vercel.app`(target: preview 확인).
> migration 없음(둘 다 app 코드만). Production 무변경. (관리자가 계정
> 정보를 잘못 입력한 경우 쉽게 수정·재발송할 수 있는 기능은 제품 오너가
> 명시적으로 "나중에" 셋업하기로 보류 — 별도 후속 배치로 남김.)
>
...
> "상담"을 "신규"로 개칭하고, "상담 현황"
> 서브탭을 "신규 현황"으로 바꿔 기존 상담 유입 + `consultation_id`가 null인
> "계정 생성"(지인 추천/직접 계정 생성 구분 없이 통일 — 사용자 요청에 따라
> `trial_onboarding_links.origin` migration은 진행하지 않음) 유입을 한
> 보드에서 함께 보여준다. 계정 생성 카드는 `consultations` 테이블에 아무
> 행도 만들지 않는다(가짜 상담 레코드 없음 — 화면 표시용으로만 기존
> `ConsultationListItem` 모양을 재사용, 단위 테스트로 `consultations.insert`
> 미호출 확인). 카드 출처 배지는 "상담 신청"/"계정 생성" 2가지. 카드 상세는
> 기존 `getConsultationCardDetailAction`의 파이프라인·계약·다음 행동 흐름을
> 그대로 재사용(신규 배치 로더 재사용, 카드별 N+1 없음) — 계정 생성 카드는
> 상담 종료 섹션만 숨긴다(상담 개념이 없으므로). 매칭 탭 하단의 "상담 →
> 체험 → 정규 전환"·"정규 계약 발송 대기"는 "신규 현황" 바로 다음의 새
> "정규 계약 발송" 탭으로 이관하고 매칭 화면에서 완전히 제거했다(같은
> 배치에서 이관+제거를 함께 함 — 이관 전 먼저 제거하지 않음). 두 탭 모두
> 관리자 계정 단위 10초 TTL 캐시(재진입 시 직전 카드 즉시 표시 후 백그라운드
> 갱신), 명시적 새로고침, 로그아웃/계정 전환 시 캐시 폐기(기존 전역 캐시
> 인프라 재사용), 최종 카드 형태 스켈레톤, 실패 시 탭 안에서 재시도 가능한
> 문구를 갖췄다. 사용자 탭의 계정 생성 폼 문구도 "지인/추천 — 상담 없이
> 바로 계정 생성" → "계정 생성"으로 통일(별도 선택 버튼 없음). **검증**:
> 신규 단위 테스트(`account-creation-kanban-merge.test.ts` 4건 — 계정
> 생성 카드만 필터링·intakeSource 태깅·가짜 레코드 미생성·카드 수 무관
> 고정 쿼리 확인), 전체 258/258 파일·1801/1801 테스트, `tsc`/eslint(신규
> 오류 없음), `next build` 성공. 로컬 프로덕션 빌드 구조 측정: 신규 현황
> 최초 진입 285ms/28요청, 재진입 205ms/28요청(SSR가 매 진입마다 재실행되는
> 기존 구조라 서버 요청 자체는 여전히 발생 — 캐시는 "화면에 즉시 표시"만
> 담당, 정규 계약 발송 100ms/1요청. 로컬 seed는 실제 계정 생성 카드가
> 없어 그 배지·상세가 실제로 뜨는지는 단위 테스트로만 확인했다 — Preview
> UAT에서 실제 계정 생성 건으로 카드 노출을 확인해야 한다.
>
> **후속 반영(2026-09-10, 같은 배치)**: "계정 생성"이 후속 계획이 아니라
> 이번 배치 범위라는 지적을 반영해 사용자 > 학부모 탭에 있던 계정 생성
> 폼(`DirectAccountCreationForm`)과 발송 내역(`DirectAccountLinksList`,
> 생성 결과·자녀별 계정 상태·초대 발송 상태·재발송 포함)을 "신규 > 계정
> 생성" 탭(새 `AccountCreationTab.tsx`)으로 옮겼다. 신규 탭 서브탭 순서는
> **신규 현황 → 정규 계약 발송 → 계정 생성** → (기존 상담 운영/체험 관리/
> 보호자 동의 대기/지난 상담/오류 재처리, 변경 없음). 사용자 탭은 이제
> 이미 생성된 학부모·학생·선생님의 조회·관리만 담당한다. 기존
> `consultation_id IS NULL` 흐름(재발급이 상담/직접 경로 자동 분기,
> `trial_onboarding_links.origin` migration 없음)과 신규 현황 카드
> 연결은 그대로 유지 — 가짜 상담 레코드는 여전히 만들지 않는다.
> `DirectAccountLinksList`도 다른 탭과 동일한 관리자 계정 단위 10초 TTL
> 캐시+최종 카드 형태 스켈레톤+재시도 문구로 바꿨다. 페이지 h1 타이틀에
> 남아있던 "상담" 문구도 "신규"로 정정했다(발견 즉시 수정).
> **검증**: 신규 테스트(`DirectAccountLinksList.test.tsx` 3건 — 스켈레톤·
> 재시도·빈 상태), 로컬 프로덕션 빌드 Playwright로 (1) 신규 탭 서브탭
> 순서, (2) 계정 생성 탭에 폼·발송 내역 정상 노출, (3) 사용자 > 학부모
> 탭에 폼/내역이 더 이상 없음, (4) 실제 계정 생성 폼 제출 → 발송 내역에
> 즉시 반영 전부 확인. 전체 259/259 파일·1804/1804 테스트, `tsc`/eslint
> (신규 오류 없음), `next build` 성공. 실제 계정 생성 뒤(보호자가 링크를
> 열어 계정이 만들어진 뒤) 신규 현황 카드 노출은 로컬 seed로 재현 불가 —
> 단위 테스트(`account-creation-kanban-merge.test.ts`)로만 확인했고
> Preview UAT에서 실제 데이터로 확인이 필요하다.
> **범위 밖(P4-3, 별도 후속 배치로 유지)**: 관리자 문서 탭(회사 문서/
> 계약/동의서)과 Google Drive 연동은 이번 배치에 포함하지 않았다 — 사용자
> 확인대로 별도 후속 배치로 남긴다.
> 커밋 `c379490`, Preview `https://alton-abcd2hcf4-alton7.vercel.app`
> (target: preview 확인).
>
> **2026-09-10 — 로드맵 업데이트(계획만, 구현·외부 변경 없음).** 현재 진행 중인
> 신규 통합 보드 배치에는 사용자 탭의 계정 생성 폼·발송/재발송 내역을 신규의
> `계정 생성` 탭으로 이관하는 범위를 포함한다. 이후 P4-3으로 관리자 문서 탭
> (`회사 문서`/`계약`/`동의서`)을 추가했다. 회사 문서는 1차로 지정 Google Drive
> 폴더의 읽기·다운로드를 목표로 하며, Drive 인증·권한·환경변수·migration은
> 별도 설계 승인 전 변경하지 않는다. 상세는
> `docs/2026-09-10-p-execution-roadmap.md`의 P0-6 후속 운영 정리와 P4-3 참고.
> 기존 보호자를 선택해 자녀만 추가하는 기능은 P4-1 온보딩 백로그로 추가했으며,
> 현재 동의·수업권·예약 P0와 C-1/C-2 이후 별도 배치로 다룬다.
> 같은 P4-1에 테스트 데이터 정리 우선의 경량 가구 아카이브·복귀를 추가했다.
> 1차에서는 기본 목록·매칭 후보·신규 보드에서 숨기고, 가구 자녀의 미래 예약은
> 취소·활성 매칭은 종료한다. 복귀해도 이 취소/종료 상태는 자동 복원하지 않으며,
> Auth 차단과 수업권·계약 변경은 실제 운영 계정 폐기 정책이 필요할 때 별도
> 확장 배치로 다룬다.
...
> 미배포.** Preview UAT에서 재진입 배치(P1-A) 배포 이후 사용자 탭
> 학부모/학생/선생님 목록이 다시 느려지고, 매칭 하단 "상담→체험→정규
> 전환" 현황표와 "과목 및 교재" 탭도 매우 느리다는 회귀 보고를 받고 조사한
> 결과: (1) 사용자 탭 회귀는 이전 배치의 `loadEmailById()` 정확성 수정이
> Preview처럼 Auth 사용자 수가 누적된 환경에서 `auth.admin.listUsers()`
> 페이지를 계속 순회하게 만든 것이 원인, (2) 매칭 현황표·과목/교재는
> 이번 배치들과 무관한 기존 코드의 오래된 N+1/무제한 조회가 데이터
> 누적으로 체감이 심해진 것. 세 가지 모두 조사·보고 후 구현했다.
> 1. **사용자 탭 이메일 조회**: `auth.users`를 id로 직접 조회하는
>    SECURITY DEFINER 함수 `get_emails_by_user_ids`(migration
>    `20261271000000`, `service_role` 전용, PUBLIC/anon/authenticated
>    실행 권한 명시적 제거)를 추가해 `loadEmailById()`가 Admin API 페이지
>    순회 대신 RPC 1회로 끝나게 했다 — 정확성 저하 없음(페이지 제한 없이
>    전달된 id 전체 조회), 대상 규모와 무관하게 항상 왕복 1회. 단위
>    테스트(빈 배열·200명 초과·중복 id 6건)와 실제 로컬 Postgres/Auth
>    통합 테스트(빈 배열·존재하지 않는 id·중복 id·250명 초과·대소문자
>    이메일·authenticated 역할 호출 거부 6건) 전부 통과.
> 2. **매칭 하단 "상담→체험→정규 전환" 현황표**: 카드마다
>    `getTrialOnboardingPipelineAction()`을 호출하던 N+1(카드당 인증+
>    순차 조회 최대 10회, 최대 50카드×10=500회)을 신규(상담) 칸반과 같은
>    패턴으로 `trial-pipeline-data.ts`의 배치 로더(쿼리 9회 고정, 카드
>    수 무관)로 교체. `getTrialOnboardingPipelineAction()`(카드 상세
>    모달 전용, 단일 후보)도 같은 배치 로더를 재사용해 로직 중복 없앰.
>    최종 카드 형태 스켈레톤 추가(빈 화면 제거). 신규 통합 보드로
>    이관되더라도 이 배치 로더를 그대로 재사용 가능.
> 3. **과목 및 교재**: `loadAllCurriculumDocs()`가 전체 문서의 섹션 본문·
>    티칭 팁·문제·키워드까지 무제한으로 읽던 것을, 목록 화면(교재 문서·
>    교재 라이브러리)이 실제로 쓰는 제목·상태·과목·단원·섹션 개수만 읽는
>    경량 로더(`loadCurriculumDocList`)로 교체(쿼리 2회 고정). 섹션 본문·
>    문제 전체는 문서를 실제로 열 때만 `getCurriculumDocDetailAction()`으로
>    조회(세션 내 캐시, 같은 문서 재조회 안 함). 교재 문서 목록에는
>    "더 보기" 방식 표시 페이지네이션 추가 — 교재 라이브러리의 과목→단원
>    드릴다운은 정확한 카운트가 필요해 목록 자체는 전체를 읽되 무거운
>    본문/문제는 아예 조회하지 않아 가볍다. 기존 편집·배포·삭제·문제
>    생성 흐름과 `/materials/[id]` URL 진입은 변경 없음(회귀 테스트 통과).
> **로컬 검증**: `tsc`/eslint(이 배치 신규 오류 없음), `supabase db reset
> --local` 정상, 전체 257/257 파일·1800/1800 테스트 통과, `next build`
> 성공. 로컬 프로덕션 빌드 구조 측정: 사용자 탭 서브탭 1~2요청,
> 교재 문서 목록 진입 시 추가 요청 0건(SSR 경량 데이터로 충분), 편집
> 진입 시에만 2요청(인증+상세 조회). **Preview에서 실제 데이터량 기준
...
> 완료(target: preview 확인).** 원인은 관리자 셸이 탭을 `if/else if` 삼항
> 체인으로 렌더링해 탭 전환마다 이전 탭 컴포넌트를 완전히 언마운트하고,
> 재진입 시 처음부터 다시 마운트·재조회하던 구조였다(문의·면담/통합
> 일정/예약은 SSR 자체가 없어 매번 클라이언트 전체 재조회, "불러오는 중..."
> 텍스트만 있는 빈 화면). 모든 탭을 `hidden`으로 유지하는 방식(초기 로드
> 비용·메모리 증가)은 채택하지 않고, 컴포넌트 트리 밖의 순수 JS 모듈
> (`tab-data-cache.ts`)에 마지막 데이터를 짧게 보관해 리마운트돼도 즉시
> 보여주는 방식을 택했다 — 재진입 시 직전 데이터를 즉시 표시하고 TTL이
> 지났으면 백그라운드로 조용히 갱신한다(신규·예약 10초, 문의·면담·통합
> 일정 30초). 캐시는 `AdminShell`이 로그인한 관리자 id에 묶고, 계정 전환
> 시 즉시 폐기, 로그아웃 클릭 시 명시적으로 폐기한다(전역 장기 캐시·
> localStorage 미사용). SSR 최초 데이터도 같은 캐시에 시딩해 최초 진입과
> 재진입이 같은 흐름을 타게 했다. 추가로 (1) 신규(상담) 탭의 카드별
> N+1(카드마다 `getTrialOnboardingPipelineAction()` 호출 → 인증+순차 조회
> 최대 10회)을 보드 전체 배치 쿼리 3회로 교체(카드 수 늘어도 쿼리 수
> 불변, 커밋 `95d4066`, Preview `https://alton-p1xqcxiab-alton7.vercel.app`
> target: preview 확인), (2) 문의·면담/통합 일정/예약에 SSR 초기 데이터 시딩 + 최종
> 표·카드 형태 스켈레톤 추가("불러오는 중..." 텍스트 제거), (3) 통합
> 일정은 고정 150일 조회 대신 "현재 표시 중인 달 ± 1개월"만 읽고 달
> 이동 시에만 그 범위를 추가 조회(이미 조회한 달은 캐시 재사용)하도록
> 축소. 예약의 8개 하위 조회는 이미 각각 100~2000행 limit이 있어 추가
> 조정 없이 SSR 시딩·캐시만 적용. 신규 보드(상담·지인 추천·직접 계정
> 생성 통합)·계약 발송 탭·커리큘럼 모델 변경·매칭 종료/재매칭 수정은
> 이번 배치에 포함하지 않았다(각각 별도 배치로 순서 합의됨). 로컬 검증:
> `tsc --noEmit`·eslint(이 배치가 새로 만든 오류 없음, 기존 무관 오류만
> `BookingReconciliationPanel.tsx`/`UnifiedScheduleTab.tsx`에 그대로 남음)
> 통과, `supabase db reset --local` 정상, 전체 255/255 파일·1789/1789
> 테스트 통과, `next build` 성공. 로컬 프로덕션 빌드 Playwright 측정(seed
> 데이터, Docker Supabase라 절대 ms는 실제 Preview 지연을 재현하지 않음):
> 4개 탭 모두 최초 진입·재진입 전부 130~370ms 안에 실제 콘텐츠 표시,
> 재진입 시 빈 화면 없음, 로그아웃→재로그인 후 신규 탭 캐시 정상 재구성
> 확인. **제품 오너가 실제로 느렸던 Preview 계정에서 최초 진입·재진입
> 전후 실측(요청 수·인증 횟수·첫 콘텐츠 표시 시간) 확인 필요** — 이
> 부분 없이는 체감 개선을 완료로 보지 않는다. 상세:
> `docs/2026-09-10-p-execution-roadmap.md` "P1 재진입 로딩 배치(A)" 절.
...
> `?child=<id>&tab=consent&focus=<subjectEnrollmentId>`를 통해 정확히
> 이동하도록 수정, 체험 리뷰 `final` 상태를 정규 진행 희망의 선행 조건으로
> 걸던 게이트를 클라이언트(`TrialConversionPanel.tsx`)와 서버
> (`confirm_regular_progress_intent` RPC, 신규 migration
> `20261269000000`)에서 모두 제거했다. 로그인 보호자 확인·가구 소유
> 확인·멱등 처리(중복 제출 방지)는 그대로 유지. 새 테이블·컬럼 없음,
> 기존 migration 파일은 수정하지 않음. 로컬 프로덕션 빌드에서 실제
> 다자녀 시나리오(Playwright)로 알림→동의 화면 이동·대상 강조·타 자녀
> 무관·리뷰 없이 선택 가능·중복 제출 방지·뒤로가기/앞으로가기 전부 확인.
> 253/253 파일·1777/1777 테스트 통과. Preview에서 다자녀 계정으로 같은
> 시나리오를 확인하기 전에는 마감하지 않는다. 상세:
> `docs/2026-09-10-p-execution-roadmap.md` P0-5.

> **2026-09-10 — Preview UAT 반영: P1-3 체감 성능 개선 미완료로 유지,
> P0-5 신규 추가.** Preview에서 상담 현황은 스켈레톤만 먼저 뜰 뿐 체감
> 로딩 시간이 이전과 같음을 제품 오너가 확인 — P1-3 1차 배치는 "중복 POST
> 제거·탭별 SSR 게이팅 완료"까지만 완료로 기록하고 "체감 성능 개선"은
> 미완료로 유지한다(코드는 되돌리지 않음, 회귀 없음). P1-2 실측 범위에
> INP(클릭→UI 갱신 지연) 측정을 추가하고, 학부모 홈 알림 클릭→목적 화면
> 경로를 요청/서버 로더/클라이언트 작업/INP 구간별로 측정하는 항목을
> 포함시켰다. 신규 **P0-5**: 학부모 "정규 진행 희망" 알림이 엉뚱한
> 화면(수강 과목)으로 연결되고 체험 리뷰 완료를 부당하게 선행 조건으로
> 거는 문제 — 조사 중, 구현 전 조사 결과·범위·migration 필요 여부를 먼저
> 보고한다. 상세: `docs/2026-09-10-p-execution-roadmap.md` P0-5/P1-2/P1-3.
...
> visit https://react.dev/errors/441..." 문구가 그대로 노출됐다(dev 모드에서는
> 같은 조작이 "이미 존재하는 키워드입니다."로 정상 표시돼 재현이 늦었다 — 응답
> 바디를 직접 비교해 dev는 `{digest, message, stack}` 전체를, prod는
> `{digest}`만 내려줌을 확인). **확정된 공통 원인**: `app/admin/subject-actions.ts`의
> `createSubjectKeyword`/`assignUnitKeyword`/`removeUnitKeyword`와
> `app/teacher/session-prep-actions.ts`의 `setSelectionActiveKeywords`가
> 검증 실패·DB 에러를 `throw`했는데, Next.js가 production에서 Server Action의
> 미처리 예외를 이 일반화된 문구로 마스킹한다 — 이미 `app/teacher/
> lesson-schedule-actions.ts`가 2026-09-06에 같은 원인으로 한 번 고친 바로
> 그 패턴이 관리자 키워드·교사 세션 준비 키워드 경로에는 아직 남아 있었다.
> **수정**: 네 함수 전부 `{ ok: true, ... } | { ok: false, error }` 형태로
> 반환하도록 변경(예외 전파 없음), 호출부(`SubjectTemplateTab.tsx`,
> `SessionPrepPanel.tsx`)를 try/catch 대신 `result.ok` 분기로 교체 — React
> 오류·스택·외부 링크는 어떤 경로로도 화면에 노출되지 않는다. 추가로 "세션
> 준비" 화면 상단에 학생명·과목명을 표시해 "이 세션"이 어느 학생 것인지
> 식별되게 하고("이 세션에 포함할 회차" → "이번 수업에 포함할 회차" + 짧은
> 설명 문구 추가), 이 식별 정보가 "학생별→운영 커리큘럼→세션 준비" 경로와
> "배정 탭→운영 커리큘럼 관리" 딥링크 경로 양쪽 모두에 전달되도록
> `CurriculumTab.tsx`/`TeacherShell.tsx`/`AssignmentsTab.tsx`의 관련 타입을
> 확장(개별 콘텐츠 선택 구조 자체는 P2-4 범위라 손대지 않음). **검증(로컬
> 프로덕션 빌드, Playwright)**: 신규 생성 → 칩 즉시 표시·#441 없음, 중복
> 입력 → #441 없이 "이미 존재하는 키워드입니다." 안내만, 새로고침 후
> 재진입 → DB에 정상 반영된 칩 그대로 유지(화면 전환 자체는 클라이언트
> 로컬 상태라 새로고침 시 목록으로 돌아가는 것은 기존부터의 동작이며
> 회귀 아님, 재확인 완료). 자동 테스트: 두 함수의 실패 경로가 안내 문구만
> 노출함을 확인하는 회귀 테스트 각 1건 추가, 기존 테스트의 throw 기반
> 단언을 `{ok,error}` 기반으로 갱신. `tsc`/`eslint` 클린(교사 화면 기존
> 무관 lint 이슈 3건은 이번 diff와 무관함을 재확인), `supabase db reset
> --local` 후 전체 스위트 252 files/1764 tests 통과, `next build` 성공.
> migration 없음, 기존 데이터 이관 없음. Production 무변경. Preview 재배포·
> 실사용자 계정 최종 확인은 별도로 진행 예정.

> **2026-09-10 — P0-3 마감(제품 오너 최종 Preview UAT 통과).** 관리자 포털의
> 탭 이동·뒤로가기·앞으로가기가 정상 동작함을 제품 오너가 확인했다(2차
> 수정 반영 후). 확인 과정에서 "URL이 바뀌는데 화면 전환에 로딩이 있어서
> 뒤로가기를 바로 누르면 더 전 페이지로 넘어간다"는 체감 지연이 남아있음이
> 보고됐으나, 이는 이력 복원 자체의 결함이 아니라(URL·화면은 결국 올바르게
> 일치) 탭 전환의 로딩 속도 문제로 판단해 **P1-1(관리자 탭 전환·상담 로딩)
> 범위로 이관**한다. P0-3은 완료. 다음은 P0-2(#441)와 P1-1을 병행.

> **2026-09-10 — P0-3 2차 원인 확정·코드 수정 완료: 공용 포털
> 내비게이션의 탭 상태·history 결함.** `/login` 가드 수정과는 별개로, 최신 Preview에서 "관리자
...
> **2026-09-10 — P0-3 최신 Preview UAT 실패: URL은 바뀌고 화면은 남음.** 관리자
> "문의·면담" 탭에서 뒤로가기를 누른 최신 Preview 재현에서 주소는 `/admin`으로
> 돌아왔지만 활성 화면은 "문의·면담"에 그대로 남았다. 앞선 `initialTab` prop 기반
> 동기화가 실제 browser popstate 경로에서 작동하지 않는 것으로 보인다. 활성 탭의
> 원본을 클라이언트 현재 URL에서 직접 읽거나 동등하게 popstate를 반영하도록 원인을
> 확정·수정한다. P0-3은 여전히 미완료이며 Production 변경 없음.

> **2026-09-10 — P0-3 최종 Preview UAT 완료.** 제품 오너가 수정 후 관리자 포털의
> 탭 이동·뒤로가기·앞으로가기가 정상 동작함을 확인했다. P0-3은 마감한다. 탭 이동
> 뒤 남는 체감 로딩은 브라우저 이력 결함이 아니라 P1 관리자 탭 전환 성능 범위로
> 분리해, 전체 초기 로더 재실행 여부를 실측하고 개선한다. Production 변경 없음.
...
> auth 복원, middleware/redirect나 캐시 복원 — 중 어느 것도 아니었다.
> `middleware.ts`의 matcher가 `/student`·`/parent`·`/teacher`·`/admin`
> 경로만 보호하고 **`/login` 자체는 보호 대상이 아니어서**, `app/login/page.tsx`가
> 세션 유무와 무관하게 항상 로그인 폼을 그렸다(코드 확인 — 원래 그런
> 분기가 아예 없었음). 그래서 로그인 직전 방문했던 `/login` 히스토리
> 항목으로 뒤로가기를 누르면, 세션이 여전히 유효한데도 그 페이지가 로그인
> 폼을 다시 그렸다 — 캐시나 OAuth 문제가 아니라 페이지 자체의 누락된 가드였다.
> **수정**: `LoginPage`가 `supabase.auth.getUser()`로 세션을 먼저 확인하고,
> 유효하면 로그인 성공 직후 이미 쓰이는 `resolveAccountDestination()`(역할별
> 홈 + 계정 상태 게이트 로직 그대로 재사용, 새 규칙 신설 없음)으로 리다이렉트한다.
> 세션이 없으면(로그아웃·만료) 기존과 동일하게 폼을 그대로 보여준다 — "로그인
> 상태 재발급"이나 "redirect 일괄 제거" 없이, 빠져 있던 가드 하나만 추가.
> 로컬 Playwright로 학생·교사·관리자 세 역할 모두 재현(수정 전) → 수정 후
> 뒤로가기 시 올바른 역할 홈으로 정상 리다이렉트(로그인 폼 미노출) 확인,
> 로그아웃 상태·로그아웃 직후 `/login` 재방문은 여전히 폼을 정상 노출함도
> 확인(회귀 없음). 신규 테스트 `app/login/page.test.tsx`에 2건 추가(유효
> 세션 시 리다이렉트, 세션 없을 때 미리다이렉트) — `tsc`/`eslint` 클린,
> `supabase db reset --local` 후 전체 스위트 252 files/1761 tests 통과,
> `next build` 성공. migration 없음, 기존 데이터 이관 없음. Preview 재배포·
> 세 역할 실사용자 계정 최종 확인은 별도로 진행 예정.

> **2026-09-10 — P0-1 마감(공유 Preview 실사용자 계정으로 확인 완료).**
> 제품 오너가 공유 non-prod Preview(`https://alton-mspiig4fl-alton7.vercel.app`)에
> 기존 관리자 권한 계정으로 직접 접속해 확인: (1) 학생 "UAT Kid 113" 홈의
> 오늘 수업 배너·9월 캘린더 배지·예정 수업 위젯·"수업" 탭이 v3 예약("테스트1",
> 9/10 19:00) 하나를 전부 동일하게 표시. (2) 교사 "Teacher test1"의 "학생별"
> 탭에서 v3 전용 배정 학생("UAT Kid 113")이 정상 노출되고 "운영 커리큘럼"
> 배지와 함께 클릭 시 정상 진입(빈 화면 아님) — "배정" 탭도 정상 확인.
> service-role 키 사용·계정 생성 우회·Production 변경 전혀 없이 기존 계정만으로
> 검증됨. **P0-1 완료.** 이 과정에서 교사 홈 대시보드가 같은 v3 예약을 누락하는
> 별도 결함(P0-4)을 새로 발견해 아래에 반영.

> **2026-09-10 — P0-4 구현 완료: 교사 홈·수업 탭 일정 일치.**
> `app/teacher/dashboard-data.ts::loadTeacherDashboard()`가 "수업" 탭의 v3
> 조회 정본(`loadTeacherLessonSchedule()`, `app/teacher/lesson-schedule-data.ts`)
> 결과를 그대로 받아 병합하도록 수정 — 별도 조회 로직을 복제하지 않음. "예정/지난"
> 판정 함수 `isPastLesson()`도 `TeacherLessonScheduleTab.tsx`에서 정본 모듈로
> 옮겨 두 화면이 공유(리뷰 미확정 체험 수업은 종료 시각이 지나도 계속 예정
> 수업에 남는 등, 판정 기준이 화면마다 갈리지 않음). 캘린더·오늘 날짜 판정도
> 서버 로컬 시간대 대신 이 교사의 `profiles.timezone` 기준(dateKeyInTimezone,
> "수업" 탭과 동일 함수)으로 통일. `app/teacher/page.tsx`는 `listMyLessonSchedule()`
> 결과가 먼저 나온 뒤에만 `loadTeacherDashboard()`를 호출하도록 순서만 조정(나머지
> 로더는 그대로 병렬). 학생 홈(2026-09-09)과 정확히 같은 패턴. migration 없음,
> 기존 데이터 이관 없음. 검증: 신규 `app/teacher/dashboard-data.test.ts`(5/5 —
> 레거시 전용/v3 전용/리뷰 미확정 체험 유지/리뷰 확정 체험 제외/자정 경계),
> `tsc`/`eslint` 클린(교사 화면 기존에 있던 무관한 lint 이슈 3건은 이번 diff와
> 무관함을 git diff로 재확인), `supabase db reset --local` 후 전체 스위트
> 252 files / 1759 tests 통과, `next build` 성공. Preview 재배포·실사용자
> 계정 UAT는 별도 세션에서 제품 오너가 진행 예정.

> **2026-09-10 — 향후 실행 계획을 P 배치로 재편(문서만).** 기존 R1~R13과
> M0~M10은 완료 이력·원래 요구사항의 출처로 보존하되, 이후 착수·승인·보고의
> 정본은 [P 실행 로드맵](2026-09-10-p-execution-roadmap.md)으로 통일한다.
> 순서는 P0(현재 UAT 결함) → P1(전반 로딩 측정·개선) → P2(키워드 기반 콘텐츠
> 운영) → P3(수업 공간·v3 커리큘럼 연속성) → P4(상담·결제·정산 운영) →
> P5(운영·소통 도구) → P6(보안·안정성·데이터 수명) → P7(파일럿·출시 게이트)다.
> P0-1은 자동 검증과 딥링크 수정까지 완료했고 공유 Preview 실제 계정 UAT만 남았다.
> 이 정리로 R8~R13에 남아 있던 세션뷰·화이트보드·Drive 재처리, 진도·마일스톤,
> 결제·환불·정산, 운영 도구, 보안·보존·복구, 출시 검증을 빠짐없이 P3~P7에
> 편입했다. 코드·migration·Preview·Production 변경 없음.

> **2026-09-10 — P0-3 추가: 포털 뒤로가기 인증 복원 결함(조사·수정 대기).**
> Preview UAT에서 학생 포털 사용 중 브라우저 뒤로가기를 누른 뒤 주소가
> `/student`로 남아 있는데 로그인 화면이 표시됐다. 유효 세션에서 이전 포털 화면을
> 복원해야 하며, 로그아웃·만료·권한 상실 시에만 로그인으로 이동해야 한다. history,
> OAuth/callback, 서버 세션, 클라이언트 auth 복원, middleware/redirect, 캐시 복원 중
...
> 저장·제출하는 "문제" 화면이 없음을 확인했다. "문제 기록"은 아카이브로 유지하고,
> P2-3에 현재 회차 문제 화면(전체·키워드·유형 필터, 객관식·서술형·풀이형 답안 저장·
> 제출, 교사 확정 전 결과 비공개)을 명시적으로 추가했다. 코드·migration·Preview·
> Production 변경 없음.

> **2026-09-10 — P0-4 추가: 교사 홈 v3 일정 누락(조사 결과 반영, 구현 대기).**
> 교사 수업 탭에는 v3 예약·세션이 보이지만 교사 홈 대시보드는 레거시
> `enrollments`/`legacy_sessions`만 읽어 같은 수업을 누락하는 것으로 확인됐다.
> P0-4에서 교사 홈과 수업 탭의 v3+legacy 병합·상태·정렬·교사 현지 시간대 기준을
> 통일하고, v3 전용·레거시 전용·혼합·자정/월 경계 Preview UAT로 닫는다. migration·
> 데이터 이관·Production 변경은 없다.

> **2026-09-10 — P0-2 Preview 재현 및 세션 준비 문구 보완(문서만).** 교사
> "세션 준비" 화면에서 키워드 선택·변경 중 `Minified React error #441`이 실제로
> 사용자에게 노출됐다. P0-2는 더 이상 재현 대기가 아니라 이 조건의 원인 확정·수정
> 단계다. 또한 "이 세션"은 특정 예정 수업(학생·일시가 있는 수업 1건)을 뜻하지만
> 화면에서 이를 식별하지 않아, 이번 수업과 참조할 커리큘럼 회차를 명확히 표시하도록
> UX 기준을 추가했다. 코드·migration·Preview·Production 변경 없음.

> **2026-09-10 — P2-4/P3 세션 준비 정책 확정(문서만).** 독립된 상단 "세션 준비"
> 화면에서 최근 회차만 고르는 방식은 쓰지 않는다. 학생 운영 커리큘럼의 각 회차
> 카드에서 키워드를 조정하고, 연결된 예정 수업을 골라 그 학생·일시가 식별된
> 세션뷰의 준비 모드로 들어간다. 키워드는 공개 교재·문제를 자동 구성하며 교사는
> 공개 콘텐츠를 개별 선택·수정하지 않는다. 다만 세션 전용 보강 자료 업로드, 수업 전
> 필기/화이트보드 초안, 과제 설정은 준비 모드에서 허용한다. 수업 시작 때 자동 구성
> 콘텐츠와 세션 전용 자료를 고정한다. 계층은 관리자 과목 기준본 → 교사 과목 운영본
> → 학생별 운영 커리큘럼 → 실제 예정 수업 세션으로 유지한다. 코드·migration·Preview·
> Production 변경 없음.

> **2026-09-10 — P2-4/P3 세 층위 세션 준비 정책 정정(문서만).** 세션 준비는
> 관리자·교사·학생별 커리큘럼 층위 모두에 존재한다. 관리자는 과목 템플릿 회차의
> 키워드·공개 콘텐츠 자동 구성 결과를 준비하는 **기준 세션 구성**을, 교사는 배정
> 과목을 자기 방식으로 조정해 학생별 커리큘럼의 기본값으로 쓰는 **과목 운영본 세션
> 구성**을, 학생별 커리큘럼은 그 학생 특수성을 반영한 **실제 수업 세션 준비**를
> 한다. 실제 세션뷰는 교사·학생이 같은 구조를 권한별로 공유하고 관리자는 운영
> 지원·점검 권한으로 열 수 있다. 세 층위는 공통 정보 구조를 쓰되 데이터 원본과
> 쓰기 권한은 분리한다. 코드·migration·Preview·Production 변경 없음.
...
> 단건 버전을 `loadStudentCreditHistoryBatch(studentIds[])`/
> `loadTeacherQcWarningsBatch(teacherIds[])`로 교체 — 각각 `.in()` 1회
> 조회 후 클라이언트에서 `student_id`/`teacher_id` 기준 그룹핑한다.
> 반환 데이터 모양·정렬(`created_at`/`occurred_at` 내림차순)·매칭 없는
> id의 빈 배열 처리는 기존 단건 버전과 완전히 동일하게 유지했다.
> `app/admin/page.tsx`는 이제 학생/교사 수와 무관하게 이력 조회 1회,
> 경고 조회 1회만 실행한다(기존: 학생 99명·교사 전원 각각 개별 호출).
>
> **테스트(신규)**: `app/admin/users-data.test.ts` — (1) 빈 id 배열이면
> 쿼리 자체가 발생하지 않음, (2) id 수와 무관하게 쿼리 정확히 1회,
> (3) 여러 학생/교사에 대한 매핑·정렬·매칭 없는 id의 빈 배열 처리가
> 기존과 동일함을 검증(학생 2건, 교사 2건 각각 실제 데이터로 확인,
> 3번째 id는 거래/경고가 전혀 없는 경우로 빈 배열 확인).
>
> **검증**: `db reset --local` → 대상 테스트 4/4 통과 → 전체 스위트
> 241/241 파일·1689/1689 테스트 통과 → `tsc --noEmit`/`next build` 클린.
> admin 페이지를 실제 로그인 후 렌더링 확인(사용자 탭 학부모/학생 목록
> 정상 표시, 런타임 에러 없음).
>
> **TTFB 참고치(제품 오너 확인: 정확한 전후 성능 보장 수치 아님)**:
> `db reset --local` 직후 seed 데이터, `admin@alton.education` 계정,
> `/admin` 하드 리로드 후 Navigation Timing API로 수정 전후 측정했다.
> **DB가 그 사이 재시딩되어 학생 수 등 절대 규모가 완전히 같지 않고,
> 웜 반복 횟수도 수정 전 1회·수정 후 2회로 동일하지 않다** — 아래
> 수치는 코드 변경의 방향성(왕복 수 99→2)을 보여주는 참고치일 뿐,
> 엄밀한 A/B 비교가 아니다. 다음 성능 후보를 다룰 때는 같은 seed
> 스냅샷·동일 반복 횟수로 다시 측정한다.
>
> | | 콜드(첫 히트) | 웜 #1 | 웜 #2 |
> |---|---|---|---|
> | 수정 전 | 1035ms | 855.7ms | (측정 안 함) |
> | 수정 후 | 612.4ms | 573.1ms | 664.6ms |
>
> 왕복 수 감소(99+회 → 2회)에 비해 로컬 측정 TTFB 개선폭은 크지 않게
> 나타난다 — 로컬 Postgres는 왕복당 지연이 매우 작기 때문으로 보이며,
> 이 차이가 측정 조건 불일치 때문인지 환경 특성 때문인지는 이번 수치만
> 으로 단정하지 않는다.
>
> **범위 준수**: 캐싱·탭별 지연 로딩·다른 포털 N+1(parent 자녀별 루프
> 등)·UI 변경은 이번 범위에 포함하지 않았다. admin 페이지 역할 게이트
> 구조, 미사용 RPC 3종, `trial_lesson_review`도 손대지 않았다. **Preview/
> non-prod 반영·UAT 계정 생성·배포·push·main 병합·외부 호출 전부 하지
> 않았다.**
...
## 2026-09-12 — P2/P3 1차 결함 수정 + 2차 교재·키워드·회차 기본 구성

기준 커밋: `2a1cd2b` 계열(아래 커밋 3건). 적용 마이그레이션:
`20261308000000_p2_unit_defaults_and_primary_keyword.sql` — 로컬과 공유
비프로덕션(`worpsqwqgnspddnrtnvq`) 모두 적용 완료.

### 1차 — UAT 차단 결함

- **준비 화면 일원화**: 수업 준비 화면은 연결된 회차의 `UnitPrepPanel`
  하나만 렌더한다. `SessionPrepPanel`과 커리큘럼의 `세션 준비 하기`
  진입점, `pinSessionSelection` export를 삭제했다(수업 시작 시 고정하는
  DB 함수는 원본 경로라 유지).
- **과목 템플릿 첫 진입**: `CatalogTab`이 자체 로드하고 로딩·빈 상태·
  오류를 구분한다(SSR prop이 탭 조건부로만 오던 것이 원인).
- **`편집` 버튼 줄바꿈** 수정.

### 선택 실패 — 범위 정정 (중요)

**현재 상태: 빈 후보의 두 원인 확인·안내 개선 / 기존 UAT 선택 실패
사례는 재검증 필요.**

확인한 것은 "후보가 비는 두 원인"이고, 그것을 화면에서 구분해 안내하도록
고쳤다(키워드 없음 ↔ 키워드는 있으나 공개 교재·확정 문제 없음).

**제품 오너가 전달한 회차에는 키워드가 선택되어 있었고 준비 자료와 수업
연결도 표시됐다.** 즉 그 사례는 빈 후보 문제가 아니다. 로컬 데이터 통계
(회차 107개 중 57개 키워드 0개)는 그 사례의 원인 증거가 아니며, 그렇게
읽었던 앞선 보고를 정정한다.

DB 층에서는 담기 → 재조회 → 수업 연결 → 준비 재진입까지 같은 자료가
유지된다(`app/teacher/unit-prep-and-start-freeze.integration.test.ts`의
`1-b`). 따라서 원인은 DB 지속성이 아니라 화면·액션 층에 있을 가능성이
높고, **아직 확인되지 않았다.** 기존 9월 14일 수업의 Preview 재검증은
미확인으로 유지한다.

### 2차 — 교재·키워드·회차 기본 구성

확정 정책: **새 회차는 키워드 없이 초안으로 존재할 수 있다.** 기존 회차에
임의 키워드를 채우거나 지우지 않는다. 자동으로 없애려 하지 않는다.

- `curriculum_docs.primary_keyword_id` — 교재당 대표 키워드 1개. 같은
  과목 검사는 트리거. **기존 교재를 백필하지 않았다**(검증: 통합 테스트).
- 기본 구성에 `position`. 지정하지 않으면 맨 뒤에 붙는다(트리거) — 기존
  쓰기 경로(`ensure_active_curriculum_overlay`, 보강 단원 조립)를 고쳐
  쓰지 않아도 되게 한 선택.
- `inherit_unit_defaults_from_template()` — 관리자 기본 키워드·교재를
  회차로 물려받는다. **선생님이 눌러야 돌고, 이미 고른 것을 덮어쓰지
  않으며, 아무것도 지우지 않는다.** 회차 생성 트리거로 걸지 않았다.
- 준비 화면: 키워드 붙이기·떼기(= 키워드 설정 진입점), 기본 구성
  가져오기(무엇이 몇 개 왔는지 알려준다), 교재 구성 순서 바꾸기·빼기.
- 관리자 과목 템플릿: 키워드 이름 수정. id 참조라 붙어 있던 연결이 그대로
  유지된다. 같은 과목 안 이름 충돌은 사유를 보여주고 편집 상태를 유지한다.

### 권한 (P4-3 정정 완료)

회사 문서·preflight·**교사 제출 서류** 모두 `requireAdmin()`으로 서버에서
관리자 자격을 명시적으로 확인한다. 비관리자는 capability가 있어도 거부된다.
capability를 OR로 받는 게이트는 이 세 경로에서 전부 제거했다. 교사 본인의
업로드·조회·다운로드 경로(`app/teacher/settlement-actions.ts`)는 그대로다.

### 남은 것

- 기존 9월 14일 수업 Preview 재검증(선택 실패 원인) — **미확인**.
- P4-3 실제 Drive 연결 검증 — 더미 문서 전용 Drive 준비 대기
  (`docs/2026-09-12-p4-3-drive-provisioning-request.md` §8 절차).
- 3차: 관리자 문제은행 독립 진입점 + 기존 AI 생성 연결.
- 4차: 문제 자동 구성, 예약 시 다음 회차 자동 연결.

## 2026-09-12(2) — 2차 잔여 반영, Drive 연결 설정

적용 마이그레이션: `20261309000000_p2_auto_inherit_and_keyword_composition.sql`
(로컬 + 공유 비프로덕션 적용 완료).

### 상속 정책 (정정 확정)

초기 상속과 기존 데이터 보정은 **다른 이야기**다.

| | 언제 | 어떻게 |
| --- | --- | --- |
| 초기 상속 | 회차를 **처음 만들 때** | 자동(트리거). 관리자 기준본의 키워드·교재가 내려온다 |
| 자동 구성 | 회차에 **키워드가 붙을 때** | 자동(트리거). 그 키워드의 기본 교재가 들어온다 |
| 보정 | 이미 있는 회차 | **수동**. `기본 구성 보충하기`를 눌러야 한다 |

"기존 무키워드 회차에 임의 값을 채우지 않는다"를 "모든 초기 상속을 수동화한다"로
읽었던 앞선 구현을 정정했다.

### 자동분과 수동분

`curriculum_overlay_unit_materials.source` = `auto` | `manual`.

- 키워드를 떼면 **auto만** 빠진다. manual은 남는다.
- 선생님이 뺀 교재는 `curriculum_overlay_unit_material_exclusions`에 남아
  다음 동기화가 되돌리지 않는다. 다시 담으면 그 기록은 지워진다.
- 자동 갱신은 기존 행의 순서를 건드리지 않고 새 것만 뒤에 붙인다.
- **기존 행은 전부 manual**이다 — 운영 중이던 구성이 키워드 변경만으로 사라지면
  안 되기 때문.

### 대표 키워드

`curriculum_docs.primary_keyword_id` + `primary_keyword_position`.
키워드의 기본 교재 = 그 키워드를 대표 키워드로 갖는 **배포된(published)** 교재
(`keyword_default_materials` 뷰). 초안 교재는 자동 구성에 들어가지 않는다.

**현재 UAT 교재가 새 후보 조회에 포함되려면**: (1) 교재가 `published`이고,
(2) 대표 키워드가 지정돼 있고, (3) 그 키워드가 해당 회차에 붙어 있어야 한다.
셋 중 하나라도 빠지면 자동으로 들어오지 않는다 — 그럴 때도 준비 화면의
`교재 목록에서 고르기`로 직접 담을 수 있다(그건 manual이라 키워드와 무관하다).

임의 백필은 하지 않았으므로 기존 교재는 전부 미지정이다. 관리자 `교재 문서`
목록의 `대표 키워드가 없는 교재만 보기`로 찾아 하나씩 지정한다. 전환으로
기존 준비 자료나 과거 수업이 사라지지 않는다 — 기존 구성은 manual이고, 과거
수업은 시작 시점 스냅샷이라 어느 쪽이든 영향을 받지 않는다.

### 남은 것 (2차 부분 완료)

- 섹션별 키워드 입력 경로 정리 — **미착수**. 대표 키워드(자동 구성용)와 섹션
  키워드(후보 검색용)는 층이 다르지만 화면에서 두 입력이 나란히 보인다.
- 3차(관리자 문제은행 + AI 생성 연결) — **미착수**.
- 9월 14일 사례 — 계속 **미확인**. 통합 테스트가 확인한 것은 그 시나리오의 DB
  경로뿐이며, 그 사례의 DB 원인이 전부 배제됐다는 뜻이 아니다.

### P4-3 Drive

Preview에 `COMPANY_DOCUMENTS_DRIVE_ID=0AAguz6mkH4-hUk9PVA`(공유 드라이브 루트),
`COMPANY_DOCUMENTS_ENABLED=true` 설정. `ROOT_FOLDER_ID`는 두지 않았다.
비관리자 차단만 실제 연결로 확인(403). 나머지는 관리자 로그인과 더미 파일이
필요하다 — `docs/2026-09-12-p4-3-drive-provisioning-request.md` §9.


## 2026-09-12(3) — 교재 변경 반영, 배포 게이트

기준 커밋 `7c0d4c15a4c28930b33715eb8737d8853e30a2df`.
적용 마이그레이션 `20261310000000_p2_resync_on_doc_change.sql`
(로컬 + 공유 비프로덕션).

- 동기화가 "회차에 키워드가 붙을 때"만 돌던 것을 고쳤다. 교재의 배포 상태·
  대표 키워드·순서가 바뀌면 그 키워드를 쓰는 회차들의 자동 구성을 다시 맞춘다.
  수동 구성·제외·순서는 보존된다. 과거 수업은 시작 시점 매니페스트 스냅샷을
  읽으므로 영향을 받지 않는다(테스트로 고정).
- 상속 트리거가 관리자 교재 순서를 잃던 버그 수정(테스트 불안정의 원인).
- 직접 담기도 배포된 교재만 허용한다(서버 검증).

### 남은 것

- **섹션별 → 교재 전체 단위 통일: 미착수.** `session_prepared_selection_content_type`
  enum이 `material_section`뿐이고, 준비 항목·매니페스트·세션뷰 읽기까지
  20개 파일이 여기 걸려 있다. 수업 시작 고정과 과거 스냅샷 조회를 함께
  검증해야 하므로, enum에 교재 단위를 **추가**하고 기존 `material_section`은
  과거 기록 조회용으로 남기는 방식으로 진행한다(기존 기록을 바꾸지 않는다).
- **3차 문제은행: 미착수.**

### P4-3 Drive 검증 상태 (정확히)

| 항목 | 상태 |
| --- | --- |
| 환경변수 설정 | 완료(Preview 전용) |
| **미로그인** 차단 | 확인 — preflight 403 |
| **로그인한 비관리자** 차단 | **미확인** — 세션이 필요하다 |
| 관리자 폴더 탐색·파일 열기·다운로드 | **미확인** |
| 빈 폴더·오류·재시도 표시 | **미확인** |

미로그인 403은 "로그인한 비관리자도 막힌다"의 증거가 아니다. 서버 코드는
`requireAdmin()`으로 역할을 확인하고 자동 테스트가 그것을 고정하지만, 실제
연결에서의 확인은 별개로 남긴다.

## 2026-09-12(4) — 문제은행, 보관 분리, 교재 단위, 예약 자동 연결

기준 커밋 `da6c1ac8756ff2687e73fc1ffc08d666c42d8319`.
Preview `https://alton-1z5lx45w4-alton7.vercel.app`.
적용 마이그레이션(로컬 + 공유 비프로덕션): `20261313000000`(문제은행),
`20261314000000`·`20261315000000`(교재 단위), `20261316000000`(예약 자동 연결).

### 문제은행 (3차)

`관리자 > 문제은행`. 교재와 독립된 진입점. 과목·상태·형식·지문 검색, 현재/보관됨
분리, 직접 쓰기와 AI 생성, 버전 기록, 초안 → 검수 요청 → 공개.

**자동 공개는 없다.** 어떻게 만들어졌든 draft로 들어오고 공개는 사람이 누른다.
초안에는 공개 버튼이 렌더되지 않고 DB도 검수 통과 버전만 받는다.

같이 고친 구멍 둘:
- `publish_problem_version()`이 `problems` 본문을 갱신하지 않아, 검수를 통과해도
  학생 화면이 옛 내용을 계속 보여줬다.
- 문제 생성 시 트리거가 버전을 함께 만들기 때문에 "새 초안 만들기"만으로는
  방금 만든 빈 문제조차 저장이 막혔다. "있으면 고치고 없으면 만든다"로 바꾸되
  검수 중인 버전은 고칠 수 없게 했다.

### 보관 목록 분리

과목 템플릿·교재 관리·교사 '내 과목'에 `현재 / 보관됨`. 기본은 현재, 검색은
보고 있는 구분 안에서만. 교재에 보관/보관 풀기를 붙였다.

**보관 과목의 교사 커리큘럼 접근을 되살렸다** — 직전 구현은 목록에서 통째로
빼버려 쌓아 둔 커리큘럼에 들어갈 길이 없었다. 보관은 숨김이지 접근 차단이 아니다.

### 교재 전체 단위

`content_type`에 `material_doc` 추가. 수업 시작 스테이징이 회차 교재 구성을 교재
단위로 넣고, 세션뷰가 그것을 모든 조각으로 펼쳐 읽는다. 검증 트리거는 교재
전체를 키워드가 아니라 "배포됨 + 보관 아님"으로 본다 — 선생님이 직접 담은 교재는
키워드와 무관한데 키워드를 요구하면 수업 시작에서 튕겨 나간다.

**과거 기록은 바꾸지 않았다.** 과거 수업의 `material_section` 행은 그대로 읽는다.

### 예약 자동 회차 연결 (4차)

새 수업에 아직 쓰지 않은 가장 앞 회차를 붙인다. 덮어쓰지 않고, 회차를 두 수업에
겹치지 않으며, 오버레이가 없으면 만들지 않고, 남은 회차가 없으면 빈 채로 연다.
연결일 뿐 고정이 아니다.

### 남은 것 / 제품 오너 확인 대기

- `/api/admin/cleanup-preview` — 관리자로 열어 JSON 전달(정리 전 집계).
- Drive **로그인한 비관리자 차단** — teacher1로 preflight 403 확인.
- 계약→수업권→정규 수업 **최소 점검**(정리의 선행조건).
- teacher1 두 사례와 9월 14일 사례: **재검증 생략, 새 데이터 기반 전체 UAT로
  대체**(해결·통과로 기록하지 않음).
- **데이터 정리는 착수하지 않았다.**
