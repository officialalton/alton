...
36. **문제은행 유형별 생성 품질 UAT(2026-09-24, 진행 중, 브랜치 `uat/problem-bank-generation-quality`)**: 위 35번 수정이 실제로 전 세부 기술·전 자료 유형에서 안전한지 SAT Math 19개(35행, 자료 유형별 분기 포함)·SAT R&W 11개(11행)를 코드로 유도한 매트릭스로 전수 점검. Math 19개는 전부 AI 없는 결정적 컴파일러 경로(`runMathCompilerBatch`)라 비용 없이 자동화(`lib/problem-generation/uat-matrix.test.ts`, vitest, 결과는 `docs/assets/2026-09-24-problem-bank-uat/matrix-result.json`) — **완료**: 35행 모두 "자료 필수인데 자료 없이 통과" 재발은 없음(35번 수정이 전 컴파일러 경로에 걸쳐 정상 작동). **새로 발견한 gap(35번 수정의 부작용, 별도 버그 아님)**: 35번 수정이 `materialBlocker`를 모든 Math 컴파일러 공통 경로에 걸면서, 애초에 그 자료 유형을 실제로 그려본 적 없는 9개 조합이 이제 0/2건 생성으로 막힘 — `linear_functions`(plane·data), `systems_linear`(figure_choice), `nonlinear_equations_systems`(plane), `ratios_rates_units`(data), `percentages`(data), `inference_margin_error`(data), `evaluating_statistical_claims`(data), `circles`(plane). 전엔 이 조합들도 조용히 텍스트형으로 새 나갔을 가능성이 높다(같은 클래스의 버그) — 지금은 조용히 안 새지만 대신 관리자가 그 옵션을 고르면 항상 실패한다. **결정 필요**: (a) 9개 컴파일러에 실제 자료 생성기를 구현하거나, (b) 지원 안 하는 자료 유형은 그 세부 기술의 관리자 선택지에서 아예 빼거나(taxonomy `alternatives`/`SKILL_DEFAULT` 축소) 정하지 않으면 이 상태로 둔다(현재는 안전하게 거부만 함, 신규 버그 아님). SAT R&W 11개 행은 실제 AI 파이프라인이 필요해 이 세션 환경에 `ANTHROPIC_API_KEY`가 없어 미실행(테스트는 자동 skip, 키만 넣으면 그대로 돈다). 브라우저 UAT(대표 문항 admin/학생 화면 캡처)도 아직 미실행 — 다음 세션에서 로컬 dev 서버로 이어서.
