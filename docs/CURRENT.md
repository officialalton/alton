...
> 여러 단원·회차에 연결(N:M 유지). 단일 단원 소속 제약·전환 대상 뷰·단원 지정 UI는 제거(`20261349000000`,
> **공유 non-prod 미적용 — 사용자가 `db push`**). 예전 단원 폴더 행은 `retired`; 제품 오너가 Drive 폴더를
> 직접 삭제하기로 함. 동기화는 기록된 폴더가 Drive 에서 사라졌으면 다시 만들 대상으로 되돌린다. 전환안은
> 설계 문서 §3.
>
> **완료한 것**
...
>
> **사용자가 할 조작(순서대로)**
> 1. `npx supabase db push --linked` (`20261349000000`) → `npx vercel deploy --yes`.
> 2. Drive 폴더 정리(직접 삭제) 뒤 Drive 자료 탭 > `폴더 동기화 실행` 2회(과목 → 키워드). 지운 과목 폴더는 다시 생긴다.
> 3. 키워드 폴더에 PDF 업로드 → 과목·키워드 선택 → `Drive 에서 불러오기` → `자료로 등록` → `공개 (고정 사본)`.
> 4. 교재 문서에 `PDF · Drive 원본` 확인 → 회차 구성에 담아 교사 `수업 준비` 교재 탭 페이지·필기, 학생 `수업 준비`·`/materials`.
