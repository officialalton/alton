...
> **2026-09-10 — B. 신규 통합 보드·정규 계약 발송 탭: 완료(신규 현황/정규
> 계약 발송 범위), Preview 배포 완료(target: preview 확인), Production
> 미적용, migration 없음.** 관리자 "상담"을 "신규"로 개칭하고, "상담 현황"
...
> **범위 밖(발견된 pending 계획 메모, 이번 배치에 포함하지 않음)**: 이
> 문서에 이미 있던 별도 메모에 "계정 생성 폼·발송 내역을 신규의 '계정
> 생성' 서브탭으로 추가 이관"·"P4-3 관리자 문서 탭(회사 문서/계약/동의서)"
> 계획이 있다 — 이번 사용자 지시에는 포함되지 않아 손대지 않았다. 다음
> 배치로 진행할지는 별도 확인 필요. 커밋 `ecca6ad`, Preview
