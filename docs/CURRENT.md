...
> **주의(미완료)**: `20261283000000` 마이그레이션은 **로컬에만 적용됐고 공유
> non-prod DB에는 아직 push되지 않았다**(`supabase db push --linked`가 이 세션의
> 권한 정책에 막힘). push 전에는 Preview의 `아카이브` 버튼·`아카이브됨` 서브탭이
> 컬럼·함수 부재로 실패한다 — UAT 전에 먼저 적용해야 한다.
> **다음**: 마이그레이션 push → Preview에서 실제 가구로 아카이브 → 목록 제외 →
> 복귀 확인(UAT 실행 ID `p4-1b-archive-20260911`). **P4-2(교사 계좌·정산)·P4-3(교사별 서류 보관)은 후속
