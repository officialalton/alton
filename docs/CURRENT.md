...
> **2026-09-06 복수 자녀 온보딩 — 완료(검수 지적 2건 정정 포함).** 최종 확정 정책(**기존 단일 칸반 유지, 별도 보드 분리 폐기**)에 따라 관리자 "체험 온보딩 안내 발송" 화면의 학생 입력을 기본 1행+`학생 추가` 반복 입력(1~N명)으로 확장했고, 보호자 링크 확인 시 시스템이 학생별 Auth 계정을 생성해 같은 household에 연결한다. **정정**: 직전 라운드(커밋 `d2a342b`)는 "학생별 칸반 카드 생성"과 "학생별 초대 상태 독립 저장"을 완료로 잘못 보고했다 — 실제로는 `consultations`에 학생별 카드를 insert하는 코드가 전혀 없었고(`family_root_consultation_id`/`is_child_onboarding_card`/`source_link_child_id` 컬럼 자체가 존재하지 않았음), 초대 상태도 `trial_onboarding_links`(가족/링크 단위) 컬럼 하나에 형제자매가 덮어쓰는 구조였다. 이번 라운드에서 두 가지를 실제로 구현했다 — 아래 "2026-09-06 검수 지적 정정" 절 참고. 이전 인수인계 문서 `docs/2026-09-06-session-handoff-consult-multichild.md`는 그 잘못된 완료 보고를 포함하므로 참고 시 이 정정 절을 우선한다.
...
- **non-prod/Preview**: 신규 마이그레이션 2개(`20261206000000_m4_multichild_trial_onboarding.sql`은 이미 직전 라운드에서 반영됨, `20261206010000_m4_student_kanban_cards_and_invite_status.sql`은 이번 라운드 신규)를 비운영 Supabase(`worpsqwqgnspddnrtnvq`)에 반영하고 `migration list --linked`로 local=remote 확인했다(결과는 실행 로그·커밋 메시지 참고). Preview는 최신 커밋으로 재배포 후 기본 HTTP 상태만 확인했다(전체 골든패스 재실행 없음). 보존 계정(`official@alton.education`/`teacher1@alton.education`) 원본 이메일·역할 데이터 그대로임을 재확인.
