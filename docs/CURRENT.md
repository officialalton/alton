...
> **2026-09-10 — P2-4/P3 공통 세션뷰 UI 정책 확정(문서만).** 세션뷰의 교재·문제·
> 필기·과제·기록과 기본 정보 구조는 관리자·교사·학생에 공통으로 유지한다. 별도
> 역할별 화면을 복제하지 않고 같은 UI를 관리자 기준본, 교사 운영본, 학생 실제 세션의
> 서로 다른 데이터 원본과 권한으로 사용한다. 관리자는 공개 기준 콘텐츠를, 교사는
> 운영본·학생 세션의 키워드와 세션 전용 준비 자료를, 학생은 배정된 결과를 학습·제출
> 권한으로 다룬다. 코드·migration·Preview·Production 변경 없음.
