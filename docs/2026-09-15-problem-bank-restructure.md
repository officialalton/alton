...
## 6. 생성 규칙·검증의 제한 조건 정리 (2026-09-15 제품 오너: "취지에 맞게")

원칙: 제한은 **① 시험 범위 밖**, **② 실제 시험 문항 관례**, **③ 표준 렌더러가 그릴 수 없는 모양** 세 가지 이유에서만 둔다. 임시로 넣은 제한은 렌더러를 확장해 없앤다. "오류가 잦아서"는 제한 이유가 아니다 — 그 경우는 생성 시 게이트 검사와 2차 자료 생성으로 처리한다.

| 제한 | 이유 | 판단 |
|---|---|---|
| 옛 형식 `coordinate_plane`·`geometry`(좌표 직접 찍기) 금지 | ③ 표준 렌더러 정책(AI 는 관계만) | 유지 |
| 지문에 방위·사분면 표현(north/south/east/west, region, quadrant) 금지 | ② 시험 문항은 라벨·점 이름으로 각을 부른다 | 유지 |
| 정답을 암시하는 보조 점·라벨·색, 선택지 좌표를 점으로 찍기 금지 | ② 정답 노출 방지 | 유지 |
| 도형 라벨은 평문(LaTeX 금지) | ③ SVG 라벨 렌더 | 유지 |
| 선택지 `x = 3` 표기·단위 기호(°, $, %) 금지 | ② SAT 선택지 관례 | 유지 |
| 지문 안 마크다운 표 금지 → figure(type:'data') | ③ 표준 렌더러 정책(검증·alt·잘림 확인) | 유지 |
| 지문에 없는 항목·라벨 금지, 값 null 금지 | ② 참조 일치·시험 자료는 빈칸이 없다 | 유지 |
| 자료 kind 목록 외(원그래프 등) 금지 | ① Digital SAT Math 자료는 표·양방향표·막대·선·히스토그램·산점도·상자그림·점도표·문장형 | 유지(근거 명시) |
| 로마숫자 진술·선택지 조합 정합, SPR 형식 | ② 시험 형식 | 유지 |
| 한 그림에 한 상황(여러 도형 섞지 않음) → 복수 자료는 figure_set | ③ 렌더 단위 | 유지 |
| 마름모 높이 미지원(대각선으로) | ③ + ② 시험은 마름모를 대각선으로 다룬다 | 유지(근거 명시) |
| 3D 좌표 | ① 시험 범위 밖 | 유지(매트릭스에 근거) |
| **횡단선끼리 만나는 점·삼각형 금지** | 임시(③ 였음) | **제거** — `parallel_transversal` 에 `crossing`(below/above)·`points on:[횡단선,횡단선]`·교점 각 region N/S/E/W 를 추가해 표준 렌더로 그린다. 검증(참조 대조·겹침·잘림)·alt 포함. 표본 `docs/assets/2026-09-14-render-samples/t1-crossing.png` |
| "복합 도형 등 이 템플릿들로 그릴 수 없는 문항" 문구 | 낡음(composite 템플릿 있음) | 문구 정정 — 실제 범위 밖만 나열(평행선 3개, 평행선에 수직인 횡단선, 셋 이상 겹친 도형) |
| `math.algebra` 규칙의 `figure(coordinate_plane)`, `math.advanced` 의 `figure(function 항목)` | 낡은 형식 이름 | 표준 `figure(type:'plane')` 로 정정 |
| 평행선 3개 | ③ 템플릿은 평행선 2개 | 유지 — Digital SAT 표본에서 3개 평행선 문항은 확인되지 않음. 나오면 확장 |
| 평행선에 수직인 횡단선(직각 표시) | ③ 템플릿은 55° 기울기 고정 | **미해결 — 결정 필요**: 실제 시험에 "line k is perpendicular to m" 문항이 있으므로 확장 후보. 확장하면 제한 제거 |
