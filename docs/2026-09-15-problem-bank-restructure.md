...
| 표 열 이름·양방향표 라벨·문장형 라벨 40자, 범례 이름 40자 | ④→완화 | 실제 시험 표 열 이름("Average Number of Tomatoes Produced per Plant")은 40자 넘음 — 생성 E2E 에서 정상 표가 거부된 사례 | HTML 로 그리는 이름 90자, SVG 범례 60자, SVG 축 항목(categories) 40자 유지(회전 렌더) | 완료(2026-09-15) | 생성 E2E 데이터 그래프 |
