...
**중간 우선순위 진행 메모(2026-09-17 — 유리식 동치)**: `equivalent_expressions`에 "유리식 동치"(분모가 다른 두 유리식을 공통분모로 결합) 서브타입 `rational_equivalence`를 신설했다(위 test9 M2-25·test10 M2-17의 "유리식(분수식) 간소화"와는 다른 패턴 — 이쪽은 단일 분수 약분, 신규 서브타입은 분모가 다른 두 항을 더하는 실제 SAT 패턴이다). 실기출과 동일하게 `A/(x-p) + B·y(x-p)/(x²y-pxy)` 형태로 출제한다 — 두 번째 항의 분모 x²y-pxy를 xy(x-p)로 인수분해해 y와 (x-p)를 약분해야 B/x가 되고, 그 다음 첫 항과 공통분모 x(x-p)로 결합해 [(A+B)x - Bp]/[x(x-p)]를 얻는다. **동치성 판정 설계**: 정답은 항상 공통분모로 결합·정리한 단일 정규형으로만 제시하고, 오답 3개는 전부 실제로 동치가 아닌 값이 되도록 계산해서 만든다(문자열 비교로 충분 — 별도의 대수적 동치 검사기는 만들지 않았다). 오답 4종 후보 중 매 생성마다 실제로 값이 겹치지 않는 3개를 뽑는다: (1) `step_missing` — 공통분모를 구하지 않고 두 번째 항을 B/x로 단순화한 뒤 분자만 그대로 더해 첫 번째 항의 분모(x-p) 위에 얹음, (2) `formula_misuse` — x²y-pxy를 xy(x-p)가 아니라 xy(p-x)로 잘못 인수분해해 두 번째 항이 B/x가 아니라 -B/x로 약분됨, (3) `step_missing` — 공통분모는 맞게 구했지만 B(x-p)를 분배할 때 상수항 p에는 B를 곱하지 않음, (4) `sign_error` — 공통분모로 결합할 때 상수항의 부호를 반대로 계산함. 렌더링은 지문·선택지에 KaTeX `$\frac{...}{...}$`를 쓰고(기존 `equivalent_expressions`의 다항식 서브타입은 분수가 없어 plain text였다), 해설은 지침대로 유니코드 텍스트만 쓴다(`$`·`\frac` 없이 "(분자)/(분모)" 형태로 서술). 기존 `EquivalentExpressionsModel`을 판별 유니온으로 확장했고(`kind: "polynomial_distribution" | "rational_equivalence"`), `generateEquivalentExpressionsModel`은 `kind`를 생략하면 두 서브타입을 50:50으로 무작위 선택한다(`nonlinear_equations_systems`의 기존 서브타입 무작위 선택 관행과 동일) — `MathCompilerSkill` 유니온이나 `batch.ts`는 변경하지 않았다(스킬 코드는 그대로 `equivalent_expressions`). 검증: `npx vitest run lib/problem-generation/math-compilers/equivalent-expressions.test.ts`(12개 테스트: 다항식 서브타입 4개 유지 + 유리식 서브타입 신규 8개 — 결합 분자 재계산·오답 3개 중복없음·numM=0 배제·오답 유형 다양성·LaTeX \frac 렌더링·해설 유니코드 전용·distractorRationales 검증, 전부 통과) + `npx vitest run app/admin lib/problem-generation` 새 실패 없음(베이스라인과 동일). `npx tsx scripts/math-compiler-validate-linear-two-var.ts equivalent_expressions`로 로컬 DB에 medium 10/10·hard 10/10 실제 저장 성공을 확인했다(배치가 다항식/유리식 두 서브타입을 무작위로 섞어 함께 생성·저장함).
