...
## 7-1. (신규, 2026-09-07) 상담 운영 화면 정리 — 레거시 UI 접기 + 부분 시간 가용시간 예외

- [ ] `상담` → `상담 운영` 화면의 월간 캘린더(예정 상담 "월간" 뷰, 공용 상담 가능시간의 "월간 실제 오픈 슬롯")가 이전보다 작고 컴팩트해졌는가(선생님 "가능시간 관리" 화면과 비슷한 크기).
- [ ] "Calendar 재처리 실행"/"Smart Notes 미매칭 재처리" 버튼이 승인 대기 신청 섹션 안의 "고급/운영 도구"(기본 접힘)를 펼쳐야 보이는가. 펼쳐서 눌렀을 때 기존처럼 동작하는가(회귀 없음).
- [ ] "Workspace Events 구독 상태" 섹션 전체(구독 목록/만료 임박 구독 갱신 실행/Smart Notes 사후 대조 실행/구독 정지·삭제)가 "고급/운영 도구 — Workspace Events 구독 상태"(기본 접힘)를 펼쳐야 보이는가. 펼쳐서 각 버튼을 눌렀을 때 기존처럼 동작하는가.
- [ ] 공용 상담 가능시간 월간 캘린더에서 날짜를 클릭하면 그 날짜의 "실제 예약 가능 슬롯"과 "오픈 시간대(반복+예외 반영)"가 함께 보이는가.
- [ ] 반복 규칙으로 특정 요일 09:00~18:00이 열려 있을 때, 날짜를 클릭해 "부분 시간대만 조정"에서 예를 들어 12:00~13:00만 "이 시간대만 휴무로" 등록하면, 그 시간대(12:00)만 "실제 예약 가능 슬롯"에서 빠지고 나머지(09:00, 11:00, 13:00, 17:00 등)는 그대로 남아 있는가. 랜딩페이지(`ConsultForm`)/보호자 포털 상담 신청 캘린더에서도 동일하게 12:00만 빠지고 나머지는 열려 있는가.
- [ ] 반복 규칙이 없는 날짜/요일에 "이 시간대만 임시 오픈으로"를 등록하면 그 시간대만 새로 예약 가능해지는가.
- [ ] "이 날짜 전체 휴무로" 버튼으로 종일 휴무를 등록하면 그 날짜 전체가 막히는가(기존 동작 유지).
- [ ] 등록한 부분 휴무/임시오픈/종일휴무 예외를 "삭제"하면 즉시 원래 상태로 복구되는가.
...
### 2026-09-07 — 관리자 칸반 "동의 요청 메일 발송" 버튼 추가

제품 오너가 실제 Preview 사용 중 발견: 상담 결과 기록(`admin_record_consultation_outcome()`)은 보호자 동의 확인(`consent_confirmed_at`)이 선행조건인데, 그 동의 확인 링크는 지금까지 상담 확정 Calendar 초대 description에만 실려 있었다. 상세 근거는 `docs/CURRENT.md`의 "2026-09-07(추가 5)" 절 참고. 이 세션은 동시 작업 중이던 다른 두 세션의 파일(`DirectAccountCreationForm.tsx`/`UsersTab.tsx`/`direct-account-actions.ts`/`DirectAccountLinksList.tsx`, `ConsultationSchedulingPanel.tsx`/`ConsultAvailabilityMonthView.tsx`)은 전혀 건드리지 않았다.

- [x] **동의 흐름 조사** — 이 동의는 `guardian_consents`/`assert_guardian_consent_ok()`(R2/R3의 13세 미만 보호자 동의 게이트, 체험 세션 생성 시 별도 체크)와는 다른 동의다. `consult_consent_versions`/`consult_consent_tokens`/`consultations.consent_version_id`+`consent_confirmed_at`로 관리되는 "상담 진행(AI 회의록/Smart Notes) 동의"이며, 만료형 토큰(`issue_consult_consent_token`, service_role 전용)으로 상담 UUID를 노출하지 않고 `/consult/consent?token=...`(`app/consult/consent/page.tsx` + `app/consult-actions.ts`)에서 보호자가 1회 확인한다. 기존 링크는 `lib/consultation/calendar-sync.ts`의 `issueConsentUrl()`이 상담 확정 시 Calendar 초대 description에만 실어 보낸다.
- [x] **`sendConsentRequestEmailAction()` 신설**(`app/admin/consultation-kanban-actions.ts`) — Calendar 초대 경로와 동일한 토큰 발급·확인 화면을 재사용해 별도 이메일 채널로 동의 확인 링크만 다시 보낸다(Calendar 초대 안의 기존 링크는 그대로 유지). 링크 URL은 `currentRequestOrigin()`으로 만든다(localhost 하드코딩 회귀 방지). 이미 동의 완료면 발송하지 않고 `already_confirmed` + 확인 시각을 반환. 실제 발송은 기존 `lib/email.ts`(`sendTrialOnboardingNoticeAction()`과 동일 패턴) 재사용, 새 발송 인프라 없음. 발송 이력은 새 테이블 없이 `consultation_status_events`(기존 INSERT-only 감사 로그, 상태 불변 이벤트)에 기록.
- [x] **UI** — `app/admin/ConsultationKanbanBoard.tsx`의 카드 상세에 `ConsentRequestButton` 추가, `!consent_confirmed_at`일 때만 "상담 결과 기록" 섹션 바로 위에 노출.
- [x] 검증: `supabase db reset --local` 성공(스키마 변경 없음, additive migration 없음) / `npx tsc --noEmit` 0 에러 / 신규 `app/admin/consultation-consent-request-email.test.ts` 4건(이미 동의 완료 시 미발송, 미확인 시 토큰 발급+실제 요청 origin 반영 링크 발송+감사 로그, sendEmail 실패 시 failed, 상담 못 찾으면 failed) 전부 통과 / `npx vitest run`(전체) — 같은 브랜치를 동시 작업 중인 다른 세션들이 반복적으로 `db reset`+전체 테스트를 돌리고 있어 완전히 조용한 시점의 단일 실행을 잡기 어려웠으나, 조용한 시점에 잡은 클린 1회 실행에서는 199파일 중 197개 통과·1306건 중 1300건 통과(실패 6건은 전부 `lib/booking/trial-entitlement-and-cancellation.integration.test.ts`, 날짜 의존 기존 known flaky, 이번 변경과 무관 파일 — 다른 세션들의 UAT 문서에도 반복 기록된 것과 동일 원인). 내가 수정한 파일(`ConsultationKanbanBoard.tsx`, `consultation-kanban-actions.ts`)과 신규 테스트 파일은 어떤 실행에서도 실패 목록에 없었다. `npx next build` 성공(32 라우트).
- [ ] 브라우저로 실제 Preview에서 동의 미확인 상담 카드에 "동의 요청 메일 발송" 버튼이 뜨는지, 클릭 후 이미 동의된 카드에서는 버튼 자체가 안 보이는지 눈으로 확인하는 것은 이번 세션 범위 밖(실제 이메일 발송 금지 제약과도 맞물림) — Preview alias 갱신 후 직접 확인 권장.
