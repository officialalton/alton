...
- [x] 문서 권한 재처리 큐 — `document_permission_retries` 큐 + `lib/documents/
      permission-retry-worker.ts` stub(claim/재시도/manual_review 전이 로직은 구현,
      실제 Drive ACL 호출은 R8에서 stub 본문만 교체).
- [ ] 커리큘럼 진도 연속 제공 확장(단원·교재·마일스톤·과제·검토 리뷰) — **정정
      (2026-09-03)**: `curriculum_handoff_status` placeholder는 실제 게이트로
      쓰지 않기로 확정(M3/R9 모두 동일). M3(선생님 배정 종료 플로우)가 이미 "날짜/
      수업유형/최종상태"만 보여주는 최소 읽기전용 과거 수업 이력을 구현했고,
      단원·교재·마일스톤·과제·검토 완료 리뷰까지 같은 범위로 확장하는 것은 R9.
...
**실행 순서(2026-09-05, M5 이후 전부 확정)**: `M0 R6 마감 → M1 상담·체험 기반 재설계 →
M2 R4 후속(체험/정규 수업권·환불) → M3 R5 후속(체험/정규 배정) → M4 상담→체험→정규 전환
통합 마감 → M5 R7(a: 판정 규칙 코어 → b: 장애·보충시간, 순차) → M6 R8∥R10(병렬) → M7 R9
→ M8 R11 → M9 R12 → M10 R13(오픈)`. 이 순서는 위 §2 표의 의존관계(R8은 R1·R7, R9는
R5·R7·R8, R10은 R4·R7, R11은 R2~R10, R12는 전 단계)를 그대로 따른 것이지 임의로 재배열한
것이 아니다 — R9(과목 마일스톤 보드)·R11(보호자–관리자 운영 메신저)의 설계 자체는 이미
각 R 섹션에 확정돼 있으므로(2026-09-03 승인) M7·M8에서 그 설계를 그대로 구현한다(재설계
안 함). 각 M 단계의 상세 범위·병렬화 근거·개발 효율화 원칙은 아래 M5~M10 절 참고.
...
- 종료 기준 재확인(2026-09-03, 조건부 승인 보완 반영): 상담 신청→관리자 수락→Calendar/Meet→
  동의→**Smart Notes 원본 자동 연결**→관리자 검토 요약 작성→상담 결과 기록까지 로컬 mock
  환경에서 실브라우저 E2E(`e2e/m1-consultation-flow.spec.ts`) 통과 + 과거 `consult_requests`와
  신규 `consultations`의 중복 정상 경로 제거(완료) + 잠재고객 레코드가 Auth 계정을 미리
  만들지 않음(완료) + Smart Notes 원본 자동 연결·"상담 완료 가능" 4개 조건(동의 확인·Smart
  Notes 활성화·원본 연결·비어있지 않은 검토 요약) 서버 강제(완료). 신규 보호자 외 상담 유형
...
- [x] 60분 체험 전용 수업권 — **지급(요구사항: "학생당 기본 1회 지급") 2026-09-03
      ...
      **미완료로 남은 하위 항목**: "관리자 예외 지급"(상담 결과 기록 경로 외의
      ...
      취소 release)는 M4(체험 예약 흐름) 선행 필요라 착수하지 않음 — 이번엔
      "지급까지만"이 승인 범위였다. **정정(2026-09-03)**: 실제로 완료된 M3는
      체험 선생님 배정·예약의 선행 단계가 아니라 이미 배정된 선생님과의 관계를
      정식으로 종료하는 흐름(termination)이었다 — 체험 예약·선생님 배정은
      여전히 M4 범위.
...
- 종료 기준: 체험 수업권이 정규 상품·Stripe 매출·환불 대상에 섞이지 않음(2026-09-03
  ...
  이번 세션에서 실행하지 않음(승인 대기, 실호출 0건). 관련 Playwright 6개 스펙
  10건(`--workers=1`) + 커밋 `6624c06` 기준 별도 clean `git worktree`에서
  build+Vitest(849/849) 재현까지 전부 통과 확인(세션 간 로컬 DB 공유 충돌로 잠시
  중단됐다가 정리 후 재개해 완료). **환불 정책·90일 유효기간 검증까지 통과해
  M2를 완료로 표시한다 — M2 완전 종료.**

### M3 — 선생님 배정 종료(termination) 플로우 — **완료(2026-09-03)**

**2026-09-03 정책 확정으로 이전 판(체험 선생님 후보/수락/거절/만료 상태 머신, 별도
trial-teacher-assignment 모델)은 전량 폐기됐다.** trial/regular는 `teacher_assignments`
레벨에서 분리된 개념이 아니라 단일 배정 관계이며(구분은 세션의 수업유형·수업권
레벨에서만), 체험 때 배정된 선생님은 계약 이후에도 그대로 유지되고 시스템은 절대
자동으로 다른 선생님을 선택하지 않는다. 아래는 실제로 구현한 범위다 — 상세는
`docs/CURRENT.md` "M3 — 선생님 배정 종료(termination) 플로우" 절과
`docs/2026-09-03-m3-migration-execution-log.md`.

- [x] 정식 "선생님 배정 종료" 요청(보호자/선생님/관리자 접수, 선생님은 요청만 가능,
      확정은 항상 관리자) → 관리자 처리(재배정 또는 수강 종료 선택) → 미래 예약
      재검증(가능시간·버퍼·중복예약)·재배정 또는 정식 취소 → 감사 이력(효과일·사유·
      요청자·처리자·후속 배정 결과) 전부 구현.
- [x] 신규 배정 선생님용 읽기전용 과거 수업 이력 접근(날짜/수업유형/상태만, 정산
      단가·Smart Notes 원본·내부 메모·다른 과목 기록 비노출) — 배정이 취소/무효화되면
      DB 함수가 매 호출마다 현재 활성 배정을 다시 검증하므로 접근이 자동으로 회수됨.
- [x] `curriculum_handoff_status`는 새 게이트로 쓰지 않고 비차단 호환 필드로만 유지.
- 종료 기준(달성 확인): 동일 `teacher_assignment`가 체험·정규 수업 모두에 쓰임(수업
  유형/수업권만 구분) + 종료 완료 후 미해결 미래 예약·hold가 남지 않도록 최종 게이트로
  차단 + 중복/동시 실행에도 중복 처리 안 됨(claim + 예약별 처리 기록) + 관리자가
  실패 상태에서 재처리 가능 + 선생님이 스스로 종료를 확정할 수 없음(구조적으로 관리자
  전용 파일에만 확정 함수 존재).
- **검증**: `e2e/m3-teacher-assignment-termination-flow.spec.ts`(관리자 종료 요청
  처리 실브라우저 검증) 신규 통과, 기존 `e2e/r5-subject-enrollment-flow.spec.ts`와
  병행 실행해도 데이터 충돌 없이 함께 통과(4 passed) — 이 E2E로 실제 버그
  1건(`subject_enrollments.status`에 잘못된 enum 값 `"ended"` 대신 `"terminated"`
...
60분 체험수업권 지급 → 배정된 선생님의 가능시간으로 체험 예약 → 체험수업·검토 리뷰 →
보호자의 정규 진행 희망 표시 → 관리자 조건 확인 → 관리자가 '정규 계약 발송' 버튼
1회 클릭 → 회사 선서명과 DocuSign 발송 즉시 실행 → 보호자 서명 완료 → 정규상품 구매 →
같은 선생님과 정규수업 계속 진행`.

확정 원칙: 잠재고객은 상담 동안 비로그인 유지 / 보호자·학생 ID는 체험 희망 확인
시점에 검증된 온보딩으로 생성(체험 이후 아님) / 이메일 문자열 자동 병합 없음 /
과목+선생님 배정은 체험 전에 완료, 체험 예약은 이미 배정된 선생님의 가능시간만
제공 / 계약 이후 새 배정을 만들지 않음(수업권·계약 상태만 변경, M3 종료 플로우를
관리자가 명시적으로 실행해야만 배정이 바뀜) / 계약·구매 완료 자체는 선생님 변경을
...
### M5 — 기존 R7 착수 *(2026-09-05, 아래 M6~M10으로 이어지는 전체 잔여 실행계획 확정)*
...
**R7 내부를 두 조각으로 나눠 진행한다** — 서로 의존관계가 있어 순차 실행:

- **M5-a(판정 규칙 코어)**: draft/scheduled/live/completed 상태 전이, 학생·선생님 취소/노쇼
  분리, 실제 시작·종료 기록, 참가자별 접속·이탈 구간(source 구분 유지), 접속기록과 최종판정
  분리, 4대 취소·지각·노쇼 규칙(24h 전 release / 24h 미만 학생취소 consume+120분 / 15분
  미접속 노쇼 확정 consume+120분 / 지각 consume+120분·보충없음)을 하나의 최종판정 함수로
  통합, `payable_minutes` 컬럼과 계산 로직(Meet 실접속시간은 정산 계산에서 제외), 선생님
  "수업 종료" 액션 + 관리자 보정 화면, 완료 처리 단일 트랜잭션, 완료 취소/재개방의 append-only
  역이력, 체험/정규 경계 준수.
- **M5-b(장애·보충시간)**: 선생님 지각분 당일 합의 연장, 미이행 지각분의 분 단위 보충시간
  생성, 90분 미만 자동 QC, 회사·Meet 장애로 미시작 시 수업권 복원·0분 정산·재예약, 장애로
  중단 시 보충시간+120분 정산 상한, 보충시간을 미래 정규 수업 뒤에 연결, 연장 구간의 선생님
  가능시간·Calendar 충돌 검사, 보충시간 적용 시 추가 수업권 미소진, 감사 이력.

M5-a가 먼저 끝나야 M5-b가 안전하다(장애 시나리오가 M5-a의 최종판정 함수를 그대로 호출·확장
하는 구조이기 때문) — 병렬 진행 금지, 반드시 순차.

### M6 — R8·R10 병렬 진행 *(M5 완료 후 착수, 서로 독립적이라 동시 진행 가능)*

R8(핵심 수업 공간 신뢰성)은 R1·R7에, R10(결제·환불·선생님 정산)은 R4·R7에 의존한다 — 둘 다
M5(R7) 완료 시점에 선행조건이 갖춰지고, 서로 건드리는 파일·테이블이 겹치지 않는다(R8은
교재/필기/Drive, R10은 payout/정산 배치) — **같은 세션에서 두 갈래로 병렬 진행**.

- **M6-a(R8)**: 세션 교재 버전 스냅샷(R9 이관분 포함), 필기 오버레이 정규화 좌표·annotation
  event·실시간 전송·재접속 복구·동시 편집 충돌 처리·전체 지우기 권한/복구, 완료 세션 읽기전용
  잠금, Shared Drive 학생→과목→연도→세션 폴더 자동 생성과 권한 자동 부여/회수, Smart Notes
  원본의 세션 폴더 이동, Drive 실패 재처리 큐(**Gate C GW-12 인수 기준 — R8이 실제 인수
  테스트를 통과시켜야 하는 항목**), 학생·보호자 ACL 없이 서버 경유 자료 제공, iPad 실기기·
  느린 네트워크 QA.
- **M6-b(R10)**: 수업권 구매·환불·차지백 대사, 중도해지 재정산, 통화·시급 스냅샷, payout
  batch 생성·검토·승인·지급·역분개, 월말 마감 일정(3영업일 명세/5영업일 이의/10영업일 지급),
  선생님 정산 상세·이의제기 화면, 매출 리포트. **환불 산식의 관할별 소비자법 검토는 정식
  오픈 전 blocker로 별도 이월**(법률 검토 필요, 이번 라운드에서 구현하지 않음).

### M7 — R9 착수 *(M6 완료 후 — R8의 Drive/annotation 구조가 선행조건)*

과목 템플릿·진도 스냅샷, `material_version_id` 채우기(R6 이관분), 교재 승인/버전, 과제
마감·제출·채점, Gemini 초안→선생님 검토→확정 리뷰(18h/24h/36h 알림), 학생·보호자 지난 수업
조회, **자녀별 과목 마일스톤 보드**(2026-09-03 승인된 설계 그대로 — 보호자 홈 대시보드+학생
읽기전용+선생님/관리자 관리 화면, 요청 버튼은 R11 운영 메신저 완성 후 실제 연결).

### M8 — R11 착수 *(R2~R10 전부 선행조건 — 이 시점에 처음 충족)*

알림 템플릿·수신자 규칙, 수업권 만료/잔여 알림, 이메일·인앱 전달 상태, 학부모 문의 케이스,
학부모-선생님 소통 정책, 학생-선생님 비공개 DM 차단+보호자 포함 과목 대화방, 관리자 전체
조회+감사 로그, 선생님 변경 시 대화방 읽기전용 보관, QC(90분 미만/Gemini 실패/36h 미확정/
지각/노쇼/민원 + 신규 선생님 첫 3회 + 무작위 5%), 실패 재처리함, 회사·계약 문서 검색·보존,
운영 대시보드. **보호자–관리자 전용 운영 메신저**(2026-09-03 승인된 설계 그대로 — household
단위 신규 테이블, `chat_threads`/`chat_messages` 패턴 재사용, 요청 템플릿별 기존 흐름 연결)도
이 단계에서 구현하고, M7의 마일스톤 보드 요청 버튼을 이 메신저에 실제로 연결해 M7~M8을 마감.

### M9 — R12 착수 *(전 단계 선행조건)*

이미 발견돼 있는 보안 항목부터 우선 처리(신규 조사 불필요, 위 R12 섹션에 원인·범위까지
확정돼 있음): 레거시 SECURITY DEFINER 함수 9개 + 프로젝트 기본 권한 규칙(anon 자동 부여)
전수 감사·revoke, 선생님 계정 정지 전 운영 게이트(R8 완료로 이제 두 조건 다 구현 가능),
테스트 데이터 격리(`is_test_data` 플래그 + 정산/매칭/통계 배제 회귀 테스트), Workspace
위임 관리자 계정 분리, `inactive` 상태 머신·복귀 UI·계정 병합 UI, retention 자동화, 감사
로그·백업 훈련·접근성/성능/브라우저 호환성.

### M10 — R13 착수 *(정식 오픈 직전, 마지막 단계)*

전체 E2E(상담→체험→계약→결제→수업→정산), 예외 시나리오 일괄 재검증, 4역할+슈퍼바이저 권한
우회 테스트, iPad 장시간 동시 접속, 데이터 마이그레이션 리허설, 운영 매뉴얼, 그리고 이미
식별된 정식 오픈 전 blocker들을 이 단계에서 전부 닫는다: `mark_expired_invites()` 스케줄러,
Production Stripe 웹훅 등록(법인·Stripe Live 활성화 후), `account-lifecycle`/`account-merge`
E2E 픽스처 격리(전역 시드 계정 의존 제거), 환불 산식 관할별 법률 검토. G6 통과 후 오픈.

---

**개발 효율화 원칙(2026-09-05, 오늘 세션 경험 반영 — M5부터 전부 적용)**:

1. **의존관계가 있는 작업은 절대 병렬로 쪼개지 않는다.** 오늘 두 번, 서로 의존하는 코드를
   동시에 두 세션이 건드려 컴파일이 깨지거나(반쪽 완료 상태) 병합 충돌이 날 뻔했다 — 병렬은
   위 M6(R8∥R10)처럼 **파일·테이블이 실제로 겹치지 않는다고 확인된 경우에만** 쓴다.
2. **작업 단위를 작게 쪼개고, 단위가 끝날 때마다 즉시 커밋한다.** 세션 사용량 한도로 에이전트가
   중간에 끊기는 일이 실제로 있었다 — 큰 덩어리를 끝까지 다 하고 한 번에 커밋하면 끊겼을 때
   전부 잃는다.
3. **에이전트의 "완료했다"는 보고를 곧이곧대로 믿지 않는다.** 실제로 `git log`/`git status`로
   커밋이 존재하는지, `tsc --noEmit`/전체 테스트가 실제로 통과하는지 직접 재확인한 뒤에만
   완료로 처리한다 — 오늘 한 번, 에이전트가 아무 코드도 안 짜고 "위임했다"는 식의 가짜 요약만
   반환한 사례가 있었다.
4. **각 M 단계가 끝나면 로컬 DB를 `supabase db reset --local`로 완전히 리셋하고 전체
   Vitest+tsc를 한 번 더 돌려 확인한 뒤에만 다음 단계로 넘어간다** — 여러 작업이 같은 로컬
   DB에 흔적을 남기면서 무관한 통합 테스트가 일시적으로 흔들리는 현상이 실제로 있었다(격리
   실행하면 통과하는 픽스처 공유 문제였음, 회귀 아니었지만 확인 절차 없이는 구분 불가).
5. **사용자가 실제로 클릭해서 확인할 수 있는 단위로 M을 쪼갠다** — 한 M이 끝나면 그 자리에서
   실브라우저로 골든패스 1회를 직접 확인한 뒤 다음 M으로 넘어간다(오늘 M4 이후 라운드에서
   이 방식으로 실제 버그를 여러 건 발견했다 — 코드 리뷰만으로는 못 잡는 것들이었다).

## R7 — 수업 상태·출석·정산 근거 — **완료(2026-09-05, 로컬 DB·테스트 기준)**

M5-a/M5-b + 제품 오너 검수 보완 라운드까지 전부 완료. 상세는 `docs/CURRENT.md`의
"M5-a(R7 판정 규칙 코어) completion"/"M5-b(R7 장애·보충시간) completion" 절과
그 이후 보완 라운드 절 참고. **로컬 개발 DB·Vitest·tsc·`next build`로만 검증됐고,
실제 Google/DocuSign/Stripe 등 외부 서비스 연동 검증(Preview/Sandbox)은 하지
않았다** — M4와 달리 이번 R7 라운드는 순수 정산·상태 로직이라 외부 연동이 필요
없었다.

- [x] draft/scheduled/live/completed 상태 전이
- [x] 학생·선생님 취소와 노쇼 분리
- [x] 실제 시작·종료 시각과 출석 기록
- [x] 참가자별 복수 접속·이탈 구간과 수집 source 저장
- [x] 접속 기록과 운영상 최종 수업 판정 분리
- [x] 시작 24시간 전까지 일반 취소 시 hold 해제·미소진·미지급
- [x] 24시간 미만 학생 늦은 취소 시 1장 소진·120분 지급
- [x] 시작 후 15분 학생 미접속과 최종 노쇼 확인 시 1장 소진·120분 지급
- [x] 학생 지각 시 예정 종료 유지·1장 소진·120분 지급·보충시간 미생성
- [x] 예약 시간과 최종 판정 기반의 분 단위 `payable_minutes`
- [x] Meet 실제 접속 시간은 정산 계산에서 제외
- [x] 수업 종료 권한과 관리자 보정
- [x] 완료 시 진도·수업권·정산 항목 단일 트랜잭션
- [x] 완료 취소 또는 재개방의 역이력 — **2026-09-05 보완**: 재판정 시 필요한
      수업권 조정을 `session_judgment_reconciliation_tasks`로 자동 생성(멱등,
      기지급 payout은 즉시 덮어쓰지 않고 R10 역분개 대상으로만 표시), 관리자가
      한 번에 반영. 원래/수정 판정·수정자·사유·시각은 감사 이력 보존.
- [x] 체험과 정규 수업 정산 규칙 — **2026-09-05 보완**: 보충시간은 정규 수업
      예약에만 적용 가능하도록 제한(체험 예약에는 적용 불가, 정책 위반 발견·수정).
- [x] 선생님 지각분의 당일 상호 합의 연장
- [x] 미이행 지각분을 분 단위 보충시간으로 생성 — **발생 후 30일 유효(제품 오너
      확정), 발생시킨 선생님의 예약에만 적용, 선생님 변경 시 승계 안 함, 만료
      기한은 어떤 사유로도 연장 불가(트리거로 차단)**.
- [x] 선생님 사유 최종 제공 90분 미만 자동 QC
- [x] 회사·Meet 장애 미시작 시 수업권 복원·0분 정산·신규 예약 흐름
- [x] 회사·Meet 장애 중단 시 미제공 분 보충시간과 총 120분 정산 상한 —
      **2026-09-05 실제 과지급 버그 발견·수정**: 지각·장애로 늘어난
      `scheduled_duration_minutes` 전액이 그대로 지급되던 것을 실제 제공
      시간(`late_start_minutes` 차감) 기준으로 수정 — 원 수업+보충 합산이
      귀책 유형별 상한(120분)을 넘지 않도록 재계산.
- [x] 보충시간을 미래 정규 수업 뒤에 연결
- [x] 연장 구간의 선생님 가능 시간·Calendar 충돌 검사
- [x] 보충시간 적용 시 추가 수업권 미소진
- [x] 지각·보충시간 QC와 감사 이력

## R8 — 핵심 수업 공간 신뢰성 *(2026-09-05 범위 명확화)*

**R8은 화면을 새로 만드는 게 아니라 이미 완성된 세션뷰(`app/session/[id]/` —
교재/필기·화이트보드/단어장/AI 문제생성, R0~R1에서 구현됨)를 실제 예약 시스템에
연결하는 작업이다.** 지금 이 화면은 `legacy_sessions`(R6 cutover 이전 테이블)만
조회하고, R6~R7이 실제로 쓰는 v3 `sessions`/`reservations`와는 연결이 전혀 없다
— 지금 실제로 예약된 체험/정규 수업에서 "수업 들어가기"를 눌러도 갈 곳이 없는
상태다. R8 범위는 다음을 **전부** 포함한다: (1) 예약·세션(v3)과 세션뷰 화면의
실제 연결(cutover), (2) 교재 버전 스냅샷, (3) 필기 오버레이 실시간 전송·영구
저장·재접속 복구·동시 편집 충돌 처리, (4) 회사 Shared Drive 폴더 구조 자동 생성·
권한 자동화, (5) Smart Notes 원본의 세션 폴더 이동, (6) Drive/Meet API 실패
재처리 큐(Gate C GW-12 인수 기준). 아래 체크리스트가 이 전체를 이미 담고 있다 —
재작성 없음.
...
- [ ] 선생님 변경 시 진도·마일스톤 기록 연속 제공 확장 *(2026-09-03 정정 —
      M3에서 이미 만든 최소 읽기 이력을 대체하는 별도 "인계" 워크플로를 만들지
      않는다)*: 별도 인계 요청/승인/완료 상태 머신·체크리스트·문서 복사·데이터
      마이그레이션·Drive 소유권 이전은 여전히 만들지 않는다. M3가 이미 제공하는
      "날짜/수업유형/최종상태"뿐인 과거 수업 이력에, R9에서 만들 단원·교재·마일스톤·
      과제·검토 완료 리뷰까지 같은 학생·과목 범위로 **읽기전용 확장**한다 — 새
      선생님은 기존 기록을 넘겨받아 소유·복사하지 않고 자신의 첫 수업부터 새
      기록을 이어서 작성한다. Smart Notes 원본·내부 메모·정산 정보·과거 대화는
      R9에서도 계속 제외. `curriculum_handoff_status`는 R9에서도 업무 게이트로
      쓰지 않는다(값은 유지하되 어떤 흐름도 이 컬럼을 조건으로 막지 않음).
...
## R10 — 결제·환불·선생님 정산 *(2026-09-05 범위 명확화 — 선생님 지급만이 아니다)*

**R10은 "선생님한테 돈 주는 것"만이 아니라 구매·환불·차지백 대사, payout batch
생성·검토·승인·지급·역분개, 리포트까지 전부 포함한다.** R7이 세션별
`payable_minutes`(정산 근거)를 만들었으니, R10은 그걸 받아 실제 통화·시급으로
계산해 batch를 만들고 지급하는 단계다. **회사 전자승인 방식(DocuSign 전자서명이
아닌 "인증된 관리자의 전자승인 기록 삽입")의 법률 적합성 검토와, 환불 산식의
판매지역별(미국·캘리포니아, 한국 등) 소비자법 검토는 이 R10 섹션 마지막 항목에
이미 명시된 대로 정식 오픈 전 blocker로 유지한다 — R10 구현 자체를 막지는
않지만, 그 항목이 법률 검토를 통과하기 전까지는 정식 오픈할 수 없다.**

**법인 설립 전 지급 경계(2026-09-07 확정)**: 법인명·EIN·사업 계좌·지급수단·
선생님 분류와 세금 절차가 확정되기 전 R10은 수업별 payout item, batch, 명세,
이의, 검토·승인과 역분개까지의 **정산 원장/시뮬레이션**만 구현한다. 실제 송금,
`paid` 상태 전환, 수취 계좌정보의 운영 수집과 지급 API 호출은 열지 않는다.
실제 지급 활성화는 법인 설립 후 계약 주체·세금서류·지급통화/수수료·지급수단을
법률·회계 검토로 확정하고 별도 승인을 받은 뒤에만 가능하다.

**지급 인프라와 승인 정책(2026-09-07 제품 오너 확정)**: 고객 수납은 Stripe,
회사의 운영 자금·정산 예비금·미국 내 강사 지급은 Mercury, 한국 강사 원화 지급은
`Mercury 정산 예비금 → Wise Business → 강사 국내 계좌`로 처리한다. 추가 전통
은행 계좌는 법인 설립 뒤 운영·보조 계좌 필요성을 별도 검토하며, R10의 선행조건도
현재 구조에 의존하지 않는다. 고객 수납, 운영비, 강사 정산금은 하나의 잔액으로
섞지 않는다.

| 구분 | 시스템상 원본과 실제 흐름 |
| --- | --- |
| 보호자 결제 | Stripe 결제·환불 원장이 원본이며, 수납금은 Mercury Operating으로 정산한다. |
| 운영비 | Mercury Operating 및 목적별 가상·법인카드에서 집행한다. |
| 미국 강사 지급 | 승인된 USD payout batch를 Mercury Teacher Payout Reserve에서 해당 강사 계좌로 ACH 지급한다. |
| 한국 강사 지급 | 승인된 KRW payout batch 재원을 Mercury Teacher Payout Reserve에서 Wise Business로 이동하고, Wise가 강사 원화 계좌로 지급한다. |
| ALTON | 지급 제공자가 아니라 정산 근거·승인·상태·명세의 원본이다. 제공자 거래 ID와 웹훅/조회 결과를 연결한다. |

**승인 후 지급 상태 머신**: `calculated → reviewed → approved → dispatch_requested → provider_pending → paid`를 기본으로 한다. 인증된 관리자 승인은 자동 지급이 활성화된 뒤 단일 서버 작업으로 `dispatch_requested`를 만들고 국가별 제공자에 지급 요청을 전송한다. 단, 관리자 승인 자체는 `paid`가 아니며, Mercury/Wise의 최종 성공 응답을 받은 뒤에만 `paid`로 전이한다. 제공자 거절·시간 초과·웹훅 유실은 `failed` 또는 재조회 대기 상태로 남기고, 동일 payout item에는 고유 idempotency key와 제공자 거래 ID를 사용해 재시도 시 이중 지급을 막는다. 부분 실패는 실패한 강사 항목만 재시도한다.

**실제 지급 활성화 Gate**: (1) 법인명·EIN·사업 계약 주체, (2) Mercury Business와 Wise Business의 회사 검증·권한·정산 예비금 계좌, (3) 강사별 국가·개인/사업자 구분·지급 통화·수취 계좌·필수 세금서류와 계약, (4) 수수료·환율·지급일·승인권자 정책, (5) 고객 판매 지역·상품 유형별 세금 추적 구조, (6) 법률·세무·회계 검토, (7) Sandbox 또는 소액 내부 지급의 멱등성·실패 재시도·웹훅 대사 검증을 모두 통과해야 한다. 이 Gate와 실제 외부 지급 활성화는 R10 구현 승인과 별개의 명시적 승인 대상이다.
...
- [ ] 법인 설립 전에는 batch를 `draft`/`reviewed`/`approved`까지만 전이시키고,
      실제 송금·`paid` 전이·지급 API를 fail-closed로 차단
- [ ] 법인 설립 후 지급 활성화 Gate: 법인명/EIN·사업 계좌·계약 주체·선생님
      분류·세금서류·지급수단·통화/수수료 정책·법률/회계 승인 확인
- [ ] Stripe/Mercury/Wise 국가별 지급 라우팅과 Mercury Operating·Teacher Payout
      Reserve 분리 — 한국 강사는 Mercury→Wise→KRW 계좌, 미국 강사는 Mercury ACH
- [ ] 승인→지급 요청→제공자 처리→완료/실패 상태 분리, 제공자 거래 ID·웹훅 대사·멱등성 키·강사별 부분 실패 재시도
- [ ] 고객 청구지 주·상품 유형(실시간 수업/디지털 콘텐츠/실물 교재/구독)·주별 매출을
      결제 원장에 보존하고, sales-tax economic nexus 검토·등록·징수의 근거로 사용
...
- [ ] 사용자 알림 설정
- [x] **(완료, 2026-09-17~18 — 아래 household 메신저·면담 항목과 함께 구현됨)**
      학부모 문의와 관리자 지원 케이스 — `household_messages`(메신저) + `meeting_requests`
      (상담 신청, 아래 "면담" 항목의 최종 사용자 노출 명칭은 `상담`으로 확정) +
      관리자 `문의·면담` 지원함(`app/admin/InquiryAndMeetingTab.tsx`)으로 구현.
      상세: `docs/CURRENT.md` 6절 "학부모 포털 IA 재구성 + 상담 신청·메신저" 참고.
- [ ] 학부모-선생님 소통 채널 정책 구현
...
- [ ] 회사 문서와 계약 문서 검색·보존
- [ ] 운영 대시보드와 핵심 전환 지표
- [x] **(완료, 2026-09-17~18 — 계획한 범위 중 핵심 구현. 아래는 원래 계획과
      실제 구현의 차이만 기록)** 보호자–관리자 전용 운영 메신저 — 기존 과목별
      `subject_threads`(선생님 채팅)와 별개로, household 단위 대화를
      신설한다. 접근: 해당 household의 활성 보호자 + 권한 있는 관리자·운영자만(학생·선생님
      접근 불가, 과목 채팅과 혼합 금지). 보호자가 시작할 수 있는 요청 템플릿: 일반 문의/과목
      ...
      따른다(새 상태 컬럼을 만들지 않는다). **실제 구현 범위(2026-09-17~18)와의 차이**:
      일반 대화(메신저)와 "상담 신청"(구조화된 단일 사유 입력 + 관리자 5단계 상태
      전이 + Calendar/Meet 연동 + 완료 후 리뷰)까지만 구현했다. 위에서 계획한
      요청 템플릿별 세분화(과목 추가/선생님 교체/환불 신청 등을 `refund_requests`
      등 기존 흐름에 연결하는 것)와 R9 마일스톤 보드 요청 버튼 연결은 **이번
      범위에 포함되지 않았다** — 필요해지면 후속으로 착수.
- [x] **(완료, 2026-09-17~18 — 사용자 노출 명칭은 "면담" 대신 "상담"으로 확정됨,
      아래는 원래 계획 문안 그대로 유지하고 명칭 차이만 표시)** 재원 중 기존 자녀의 학습·일정·선생님·
      서비스 현황 논의는 신규 가입/새 자녀 온보딩 `상담`과 분리해 `면담`으로 부른다. 보호자는
      위 household 메신저의 문의 대화에서 실시간 논의가 필요할 때 `면담 일정 요청`으로 전환하고,
      관리자는 가입용 상담 칸반과 섞지 않는 별도 `면담 운영` 화면에서 접수·일정·완료 이력을
      관리한다. 일정 UI는 랜딩/자녀 추가 상담에서 쓰는 공통 월간 캘린더→날짜→시간 선택
      컴포넌트를 재사용하되, 면담 데이터·상태·가용시간은 가입 상담과 분리한다. 면담 시간,
      가용시간 규칙, Smart Notes/동의 적용은 R11 착수 시 확정하며 그 전에는 빈 탭이나 가짜
      동작을 만들지 않는다. **실제 구현과의 차이**: 일정 확정 UI는 랜딩의 공용
      월간 캘린더 컴포넌트를 재사용하지 않고 관리자용 `datetime-local` 인라인
      입력으로 구현(가용시간 규칙 자체는 기존 `meeting_availability_rules/
      exceptions`를 그대로 씀). 완료된 상담에 대한 Smart Notes 자동 연동은 만들지
      않았다 — 대신 관리자가 리뷰 확정 시 미팅록 Drive 문서 id를 수동으로 연결하고
      그때 `grantSmartNotesReaderPermission`(세션 Smart Notes와 동일 헬퍼)으로
      권한을 부여하는 방식으로 구현.
- [x] **(2026-09-06 계획 → 2026-09-17~18 최종 구현 시점에 (1)과 마지막 문단이
      제품 오너 지시로 뒤집힘 — 아래 "실제 구현" 문단이 최신 정책, 나머지는
      원안 그대로 보존)** 위 `면담` 항목의 세부 범위를 다음과 같이 못 박는다: (1) 면담은 자녀·과목을 선택해
      요청하며, 월간캘린더→시간선택 UI는 신규 가입 `상담`과 **UI 컴포넌트만** 공유한다(향후
      공용화 예정 — 이번 라운드에서는 실제 공용 컴포넌트를 만들지 않았다. 이번 라운드가 고친
      학생별 칸반 카드 작업은 `상담` 단일 칸반 유지 정책이며, `면담`은 애초에 그 칸반과 무관한
      별도 도메인이므로 서로 충돌하지 않는다). (2) 상담과 면담은 데이터·상태·운영 보드를
      분리한다 — 관리자는 `상담` 칸반과 별도로 `면담 운영` 화면에서 접수·일정·완료 이력을
      관리한다(위 항목과 동일, 재확인). (3) 비동기 메시지만으로 해결된 문의는 면담 일정으로
      전환할 필요가 없다 — household 메신저 대화만으로 종결 가능. (4) 기존 재원 자녀의 면담은
      가입·체험·정규 전환 파이프라인 카드(상담 칸반, 온보딩 링크, 학생별 진행 카드 등)를
      **절대 생성하지 않는다** — 그 파이프라인은 신규 가입/신규 자녀 온보딩 전용이다. (5)
      면담 시간·가용시간 규칙·Smart Notes 적용 여부는 R11 착수 시 확정하며 임의 구현하지
      않는다(위 항목과 동일, 재확인). 랜딩/보호자포털 공용 캘린더 컴포넌트, 보호자 포털의
      "자녀 추가 상담" 셀프서비스(관리자 대신 보호자가 직접 새 자녀 상담을 신청하는 흐름)는
      **실제 구현(2026-09-18 최종 정책)**: 자녀·과목 선택 자체를 없앴다 — 학부모 상담
      신청 폼은 `상담 사유` 단일 텍스트 입력만 받는다(위 (1)과 다름, 제품 오너가
      나중에 더 단순화하기로 결정). 신규 자녀를 위한 별도 셀프서비스 흐름
      (`ConsultRequestTab.tsx` 등)은 한때 만들었다가 이 최종 단순화 결정으로
      완전히 폐기했다 — 신규든 기존 자녀든 학부모는 `상담 신청` 서브탭 하나만
      쓴다. 나머지(2)~(5)는 원안 그대로 구현됨(상담/면담 보드 분리, 파이프라인
      카드 미생성 등). 랜딩/보호자포털 공용 캘린더 컴포넌트 공용화는 여전히
      범위 밖으로 남아있다(`docs/CURRENT.md`에도 동일하게 명시).
...
- [ ] 복수 자녀·복수 보호자·복수 과목
- [ ] **(2026-09-06 온보딩 UX 후속 백로그)** 현재는 관리자가 체험 온보딩 발송 화면에서 학생
      1~N명의 이름·이메일·학년·과목을 입력하고, 보호자가 하나의 링크를 확인하면 시스템이
      학생별 계정을 생성·연결한다. 향후에는 보호자가 그 링크에서 자녀 수와 자녀별 정보를
      직접 입력·확인하도록 입력 주체를 전환한다. 기존/신규 보호자 재사용, household 연결,
      학생별 초대·단일 칸반 카드 분리·멱등성은 그대로 유지하며 입력 주체만 바뀌어야 한다.
