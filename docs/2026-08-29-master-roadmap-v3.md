...
- [ ] **(R2 Task 7 로컬 구현·검증 완료 2026-09-05, 실제 인프라 검증 대기 — mock만으로 완료 처리하지 않음)** 선생님 퇴사 시 Workspace·Docs·채팅·향후 세션 권한 회수 — Google Workspace OAuth 로그인 + `teacher_workspace_provisioning` staging + 7개 활성화 선행조건(각각 별도 증거·시각) + 정지(`suspend`, 삭제 아님)/재활성화 전부 구현·psql/vitest/Playwright로 로컬 검증 완료. 남은 것: GCP/Vercel WIF 연결·Directory API scope 추가·테스트 OU 생성 등 인프라 사전조건(사용자 조치 필요) 완료 후 실제 1회 검증(테스트 OU 계정 생성, 충돌·재시도·정지·재활성화, 실제 OAuth 로그인). 상세는 `2026-08-29-r2-migration-execution-log.md` Task 7 참고.
