...
**빈 양식(작성 전 서식)은 어디에 두는가**: 교사가 받아 갈 빈 W-9 양식은
제출물이 아니라 회사가 배포하는 문서다 — `문서 > 회사 문서`(§5, Drive 읽기
전용)에 둔다. 제출물 보관함(§6)에 빈 양식을 섞지 않는다. Drive 폴더 구조
(§9-5)는 현재 `법인 서류/`, `계약서 양식/` 두 개인데, 교사 배포용 서식이
`계약서 양식/` 아래에 들어갈지 별도 폴더가 필요할지는 §9-13에서 결정한다.
...
아직 결정되지 않은 것은 §9-13에 모아 둔다(보존 기간, 열람 가능한 관리자 범위,
서식 종류 고정 여부).
...
13. **미결정(제품 오너 결정 필요, 구현 착수 전)** — W-9류가 납세자번호를
    담는다는 점 때문에 생긴 항목이다(§6.0.1).
    ① **서식 종류를 고정할 것인가**: 현재 P4-2는 종류 없이 자유 업로드다.
    `W-9` / `W-8BEN` / `기타`처럼 고정 목록을 둘지, 자유 업로드를 유지할지.
    고정하면 `teacher_documents`에 종류 컬럼을 추가하는 additive migration이
    1건 늘어난다(그래도 상태 컬럼은 만들지 않는다).
    ② **열람 가능한 관리자 범위**: 현재 설계는 `requireAdmin()`(관리자 전원)이다.
    납세 서식만 더 좁은 범위(예: 대표·정산 담당)로 제한할 필요가 있는가.
    ③ **보존 기간**: 지금은 자동 삭제를 만들지 않고 P6 개인정보 보존·삭제
    정책으로 넘긴다. 세무 서류에 별도 보존 연한이 필요한가.
    ④ **교사 배포용 빈 서식의 Drive 위치**: `계약서 양식/` 아래에 둘지 별도
    최상위 폴더를 만들지(§9-5의 2폴더 구조 변경 여부).
14. **다운로드 감사 테이블은 계약과 공용** — `document_access_events` 1개를
