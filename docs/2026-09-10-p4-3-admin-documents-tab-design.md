...
**2026-09-12 제품 결정으로 둘 다 확정됐다**: 열람 범위는 관리자 전원이 아니라
**`정산 담당 관리자`만**(§9-9), 보존 기간 자동 삭제는 **이번 범위 밖이며 P6에서
결정**한다(§9-15). 서식 종류도 강제하지 않는다(§9-13). 남은 미결정 항목은 없다 —
다만 **P4-3 구현 착수 자체는 별도 지시를 기다린다.**
...
- **앱 게이트: `정산 담당 관리자` 권한(2026-09-12 제품 결정으로 변경).**
  `requireAdmin()`(관리자 전원)이 아니라
  `requireAdminOrCapability('정산권한')`**만으로도 부족하다** — 요구는 "일반
  관리자에게는 열람 권한을 주지 않는다"이므로, **`role='admin'`이라는 이유만으로
  통과시키지 않는** 별도 게이트가 필요하다. 구현 시 `정산권한` capability를
  보유한 사용자만 통과시키는 전용 헬퍼를 쓴다(기존 `requireAdminOrCapability`는
  `is_admin() OR capability`라 관리자 전원을 통과시키므로 그대로 쓰면 안 된다).
  계약·동의서는 종전대로 `requireAdmin()`이다 — 서브탭마다 게이트가 다르다.
...
9. **교사 서류 접근 권한(2026-09-12 제품 결정으로 확정·변경)** — 관리자 전원이
   아니라 **`정산 담당 관리자`만** 열람·다운로드할 수 있다. 일반 관리자에게는
   열람 권한을 주지 않는다. 즉 `requireAdmin()`도, `is_admin() OR capability`
   형태인 기존 `requireAdminOrCapability()`도 쓰면 안 되고, **`정산권한`
   capability 보유 여부만 보는 전용 게이트**가 필요하다.
   관리자 조회는 service_role로 RLS를 우회하므로 **이 앱 게이트가 유일한 실제
   통제**다. 민감 서류의 **열람·다운로드는 반드시 감사 기록을 남긴다**(§7.1).
   `teacher_documents` RLS의 `정산권한` 허용은 교사 본인·정산 담당자의 직접 조회
   경로용으로 그대로 두고, 좁히는 migration은 만들지 않는다.
...
14. **빈 서식의 위치(확정)** — **새 최상위 폴더를 만들지 않는다.**
    ...
    빈 양식은 계속 분리한다.
15. **보존 기간(2026-09-12 확정)** — **보존 기간 자동 삭제는 이번 범위에 넣지
    않는다.** 파일은 우선 보관한다. 법인 소재지·세무·개인정보 보존 정책을
    확정하는 **P6**에서 보존 기간과 삭제 절차를 함께 결정한다. 따라서 이 설계에
    삭제 스케줄러·만료 컬럼을 만들지 않는다.
