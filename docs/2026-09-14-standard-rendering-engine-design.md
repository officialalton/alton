...
>
> **템플릿 4(표·데이터 그래프) 구현 완료** — `templates/data.ts`: 한 원본(type 'data')에서 표(<table> 시맨틱)·숫자 목록·막대(다중 계열+범례)·선·히스토그램(이어진 구간)·산점도+추세선·상자그림. 렌더러가 축 범위·눈금·막대 폭·범례·범주 라벨(넘치면 45° 기울임, 그래도 넘치면 거부) 을 정한다. lint: 지문의 따옴표 항목·"the X column/row/bar" 존재, [행/범주 + 열/계열 + 숫자] 절의 값 일치(`Shift 4 … 14 defective` ↔ 표 값), 단위 낱말(dollars, hours …)이 열 이름·축 제목에 있는지, 지원하지 않는 kind(원그래프 등)·행 길이·범례 없는 다중 계열·끊어진 구간·상자 순서 오류는 스키마 거부. AI 가 `{kind:'table', table:{…}}` 로 감싸 보낸 실례 → 정규화로 수용. `require_data` 정책(Problem-Solving and Data Analysis 유형 기본), 초안 버튼 `AI로 표·그래프 데이터 만들기`. 표본 `t4-desktop.png`, E2E `e2e/10~12-*.png`.
>
> **보완 구현 완료(2026-09-14 저녁)** — 데이터: 점도표(`dot_plot`)·양방향 표(`two_way`, 합계 자동)·문장형 자료(`statement`, 표본·오차범위 값 참조 검증). 좌표평면: 음영 부등식(`inequality`: 실선/점선·음영 다각형·공통 영역), 조각함수(`piecewise`, 열린/닫힌 점), 유리함수(`fn:'rational'`, 점근선 점선), 선택지 부등호·경계 대조(`option_mismatch`), 선택지 좌표를 점으로 찍으면 정답 노출 거부(`option_leak`), 눈금 숫자는 간격 배수에만. **템플릿 5 원** 구현: 점은 각도로 자리만, 반지름·현/지름·호·부채꼴·중심각·원주각·접선(⟂ 표시), 참조 lint(center, chord/diameter/arc, tangent at, ∠, 길이·각 라벨). 표본 `t5-desktop.png`, E2E `e2e/13~18-*.png`.
>
> **템플릿 6·7 구현 완료(2026-09-14 저녁)** — `templates/polygon.ts`(직사각형·정사각형·평행사변형·마름모·사다리꼴·정n각형: 표준형 배치, 변 라벨/눗금, 각 호/직각(직사각형·정사각형 자동), 대각선(1/4 지점 라벨), 높이 점선+직각; lint: 도형 종류 낱말, 도형 이름 글자, side/diagonal/`AB = 6`, ∠, 각도 라벨), `templates/solid.ts`(직육면체·정육면체·원기둥·원뿔·구·사각뿔 2.5D, 숨은 모서리 점선, 치수 라벨 자리 규칙(후보 중 겹치지 않는 곳), lint: 종류 낱말·radius/diameter/height/slant/edge/length/width 값 대조, 미지수 한 글자 허용). 표본 `t6-desktop.png`, E2E `e2e/19~24-*.png`(사다리꼴 넓이, 원기둥 부피 — 생성→검증→공개→학생 화면 통과).
>
> **좌표기하·복합 도형 구현 완료(2026-09-14 저녁)** — 좌표평면 객체에 `polygon`(점 id 꼭짓점·채움)·`circle`(중심 id·반지름, 등축 스케일 자동)·`midpoint`·`intersection`(파생 점, 평행이면 거부)·`transform`(평행이동·대칭 x/y/y=x/y=−x·확대·회전 90/180/270, 상은 점선+프라임 라벨) 추가. lint: 지문의 기울기·길이(`AB = 5`, `√20`)·중점 좌표·넓이(신발끈)·평행/수직·평행이동(units right/up)·대칭 축·확대 배율을 좌표로 **계산해 대조**, 도형 이름 글자 존재, 좌표는 점/도형 위. 라벨 후보 세 고리. 대표 8문항 추가(총 좌표평면 21). 표본 `t3-desktop.png`.
>
> **그래프/도형 선택지·복수 자료·수식/선택지 블록 구현 완료(2026-09-14 저녁)** — `figure_choice`(선택지 4개 그림: 같은 축·객체 수·kind, 라벨/점 금지 `choice_bias`, 선택지 수 대조, 2×2 격자(모바일 1열) A~D 캡션, 학생 화면은 선택지 칸 안에 그림, 정답은 `correct_index` 로 분리 — 순서를 바꾸려면 choices 와 options 를 함께 바꿈), `figure_set`(자료 A/B id 분리, 'Figure A' 참조 검증). 수식·선택지 블록: `lib/problem-content-check.ts` — KaTeX 실제 조판으로 파싱 실패·닫히지 않은 $·수식 밖 LaTeX 노출을 **저장 거부**, 선택지 수·중복·정답 범위·'x =' 표기, 로마숫자 진술(`statements`, `20261368`)과 조합 선택지(`I only`, `I and II`, `Neither`) 정합, SPR 정답 형식·자릿수 → 내용 문제도 `render_check` 에 실려 공개 게이트가 막는다(그림 없는 문항 포함). 학생 화면 진술 블록(I. II. III.), 관리자 진술 입력·즉시 사유. 연립방정식 `\begin{cases}`·부등식 연쇄·구간·분수·근호는 KaTeX 로 조판(테스트). E2E `25-st-*`(진술 블록 공개→학생).
>
> **커버리지 기준선**: `docs/2026-09-14-sat-math-coverage-matrix.md` — 세부 기술 코드 19개 × 형식 × 자료 블록/템플릿 × 검증 상태(검증됨 16 / 부분 3), 표현 종류별 상태, 미지원 명시. figure_choice 는 생성 뒤 정답 그래프를 correct_index 자리로 옮기고(`placeCorrectChoice`) 정답 불일치·중복 정답을 거부(`answer_mismatch`); AI 변형 모양(xmin/xmax·type·points)은 정규화로 수용.
>
> **RW 구조화 자료 블록(2026-09-14 밤)** — 기존 11개 RW 유형: Text 1/Text 2 제목 구역, 메모 목록+목표, 빈칸(______)·밑줄(__문장__) 대상 정확히 하나 + 질문 참조, 질문 단락 분리, 정량 근거는 figure(type:'data')(마크다운 표 거부). 지문은 `passage` 그대로 두고 저장 시 해석·검증해 `render_check` 에 싣는다(코드 없는 옛 문제는 검사하지 않음). 학생·관리자 같은 렌더(`RwStimulusView`). 로컬 E2E 13건 통과. 상세 `docs/2026-09-14-rw-structured-blocks.md`.
>
> **SAT Math 마무리(2026-09-14 저녁)** — 매트릭스 '부분' 3건 해소(비선형 연립 교점·통계적 주장·포물선 선택지: 표본+E2E), E2E 없던 표현 5건(도형 선택지·복수 자료·좌표기하 변환·막대그래프·복합 도형) 모두 AI 생성→공개→학생 통과. **템플릿 8 composite**(도형 안 도형 음영: 정사각형 안 원, 원 안 정사각형/삼각형, 직사각형 위 반원, 테두리) 추가 — 복합 겹침 도형은 시험 범위 안으로 판정. 3D 좌표는 범위 밖(근거 명시). figure_choice 는 이차식(일반형·꼭짓점형)까지 정답 자리 대조, 도형 자식(삼각형 등) 허용. 매트릭스: 19개 코드 전부 검증됨 → "SAT Math 범위 완료(로컬 E2E 기준)".
>
...
> **로드맵 보완(2026-09-14 저녁, SAT Practice Test 6~11 대조)** — SAT Math 표현 범위를 빠짐없이 닫기 위한 추가(기존 범위 변경 아님):
> 1) 표·데이터: 점도표(dot plot), 양방향 표/조건부확률 표(two-way), 가중평균·표본 추정·오차범위·관찰/실험 주장 판단용 **문장형 자료 블록(statement)** — 같은 데이터 원본·지문 참조·값 검증.
> 2) 좌표평면: 부등식 그래프(실선/점선 경계·음영 방향·복수 부등식 공통 영역), 조각함수와 열린/닫힌 점, 유리함수·정의역 제한·점근선 표준 표현, **그래프 선택지 4개를 같은 축·눈금·크기로 나란히**(figure choice).
> 3) 기하·측정: 원(중심·반지름·현·호·부채꼴·접선·중심각), 교차 선분·복수 도형·좌표기하, 입체(직육면체·원기둥·원뿔·구 2.5D, 숨은 모서리 점선, 치수 라벨), 넓이·겉넓이·부피는 도식과 참조 일치.
> 4) 모든 새 표현에 같은 검증(참조·값·단위·축·범례·라벨·충돌·잘림·모바일)과 분류(SAT 영역·기술 코드), 대표 문항·거부 사례·로컬 E2E·Preview 게이트.
