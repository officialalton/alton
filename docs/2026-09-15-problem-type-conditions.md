...
- 통과 4/24(17%) · 재생성 36회(해소 1). 어려움 기준(무관·명백 오답 하나도 불허, 추정 난이도 hard)은 제품 오너 지시대로 엄격하며, **현재 생성기는 이 기준을 대부분 통과하지 못한다** — 지시에서 지적한 "지문 길이·어휘만 어렵고 오답이 쉽게 지워지는" 문항이 실제로 대다수라는 뜻이다. 게이트가 그런 문항을 저장하지 않는 것은 의도된 동작이다.
- 반복 사유: 어려움 문항의 오답 1개가 무관(20)·명백(15); 선택지 5개(5); 좌표평면 참조·라벨 겹침; 도형 유형은 생성 응답에 문항이 없던 경우 2회.
- 조치: 스키마에 객관식 선택지 `minItems/maxItems 4` 강제, 어려움은 사유 피드백 재생성을 2회까지 허용, 오답 유형 다양화 규칙. 재실행 결과는 아래 D-4 에 기록한다. 그 전까지 어려움 문항은 통과율이 낮아 관리자가 요청 개수보다 적게 받을 수 있다(부족분 재생성 1회 포함).

### D-4. 4차 배치 — 어려움 재실행 (실행 뒤 갱신)
