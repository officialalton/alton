...
- [ ] Implement the migration (including the `SECURITY DEFINER` function and the RLS/grant setup that gives ordinary roles SELECT-only on the manifest table) + `session-content-data.ts`'s manifest reader, run tests, `supabase db reset --local`, `tsc --noEmit`; commit separately from Task 1.
...
(1) a session's `session_content_manifest` is an exact copy of the teacher's explicitly staged and
