...
> **인증 구현 교체(2026-09-05)**: 콘솔 설정 착수 전 사용자가 인프라 체크리스트를 검토하며 인증 구현 자체를 재점검했다. Vercel 공식 문서(`https://vercel.com/docs/oidc/gcp`)가 OIDC→WIF→impersonation 구간에 `@vercel/oidc`의 `getVercelOidcToken()` + `google-auth-library`의 `ExternalAccountClient`(`subject_token_supplier`/`service_account_impersonation_url`)를 쓰도록 명시하고 있어, 이전에 raw fetch로 손수 구현했던 STS 교환 코드를 완전히 제거하고 이 공식 경로로 교체했다(fallback 없이 대체 — 같은 보안 경로가 두 개면 검증·유지보수가 어려워진다는 판단). `signJwt`(DWD) 이후 구간은 Vercel 문서가 다루지 않는 Workspace 전용 로직이라 기존 커스텀 구현을 그대로 유지한다. 동시에 read-only preflight와 write provisioning을 **파일 단위로 구조 분리**했다(`lib/google-workspace-directory-readonly.ts`는 GET만, `lib/google-workspace.ts`는 쓰기만 — 같은 함수에서 boolean 하나로 가르지 않는다) — 상세는 `2026-08-29-r2-migration-execution-log.md`의 "Task 7 — 인증 구현 교체" 절 참고. 아래 "신규 데이터 필드"·"구현 순서" 등에 남아있는 `lib/google-workspace.ts` 단일 파일 서술은 이 교체 전 표현이며, 실제로는 `lib/google-workspace-auth.ts`(인증 체인)+`lib/google-workspace-directory-readonly.ts`(읽기)+`lib/google-workspace.ts`(쓰기) 3개 파일로 나뉜다.
...
- [x] 실제 인프라(2026-09-01 전부 완료, 상세는 실행 로그 "read-only preflight 최초 성공"·"실제 쓰기 검증 완료, Task 7 완료 처리" 절 참고): (a) DWD Client ID scope 확인 완료, (b) Vercel OIDC ↔ GCP WIF 실제 연결 검증 완료(도중 `context.audience` 오탐지 버그 발견·수정), (c) 테스트 OU에서 실제 Workspace 계정 1건 생성 완료(`teacher-provisioning-test@alton.education`), (d) 실제 재시도 시 중복 생성 없음(DB 가드로 Directory API 도달 전 차단) 확인, 정지·재활성화 실제 검증 완료(도중 `transition_account_status()` 게이트가 `pending→active`에만 걸려 있던 정책 버그 발견·수정 — `active`로 가는 모든 전이에 적용하도록 확장), (e) 실제 Google OAuth 최초 로그인 + immutable Google user ID 연결 검증 완료(`teachers.status`가 연결 직후 `pending` 유지, 7개 선행조건 충족 전까지 재활성화도 차단됨을 실측). 정리 단계에서 `teacher_rate_history`/`account_status_events`의 하드 immutable 트리거로 테스트 profile을 물리 삭제할 수 없음을 발견 — 트리거 우회 없이 `suspended`로 영구 보존하기로 사용자 확정, `master-roadmap-v3.md` R12에 안전한 테스트 데이터 정리 절차를 후속 작업으로 등록. **Task 7 완료.**
