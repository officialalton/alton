...
## 10. 5/N — 역할별 UI 폴리싱 + 외부 통합 검증 요청서 초안 (커밋 `29e13e3`)

관리자 14단계 파이프라인 시각화, 선생님 리뷰 초안/미리보기/공개확정 분리,
보호자 화면 안내 강화, 학생/보호자 공용 확정 리뷰 표시 분리, 디자인 토큰
통일. 상세는 `docs/CURRENT.md` M4 절 5/N 항목 참고.

## 11. 6/N — 온보딩 이메일 실제 전달 경로 + 로그인 이메일 분리 (커밋 다음 참고)

체험 온보딩 안내를 기존 SMTP 경로로 실제 발송(로컬 Mailpit만, 절대 URL·
발송시각·상태·내용 지문 기록, 중복 발송 방지, 실패는 계정 생성과 무관하게
별도 상태), prospect 이메일과 보호자 로그인 이메일 분리(변경 시 별도 소유
확인, 자동 병합 없음, 원본 스냅샷 보존). 외부 통합 검증 요청서를 Preview
브랜치 방식·원격 DB 마이그레이션 계획·Google/DocuSign/Stripe 범위 축소·
23단계 UAT·확정 테스트 계정으로 갱신. 상세는 `docs/CURRENT.md` M4 절 6/N
항목 참고.
...
- `29e13e3` — 5/N: 역할별 UI 폴리싱 + 외부 통합 검증 요청서 초안.
- (이번 커밋) — 6/N: 온보딩 이메일 전달 경로 + 로그인 이메일 분리 + 요청서 갱신.
