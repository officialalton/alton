...
export const getAnthropic = () => (anthropicClient ??= new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }));
...
const FIGURE_POLICY_RULE: Record<FigurePolicy, string> = {
  ...
  require_data: "**모든 문항에 figure(type:'data') 자료가 있어야 한다** — 표(함수표·값 표·빈도/비율표)·숫자 목록·막대/선그래프·히스토그램·산점도(추세선)·상자그림 중 문항에 맞는 하나. AI 는 값·이름·단위만 내고 그림은 그리지 않는다. 지문은 자료를 읽어야 풀 수 있게(비율·백분율·단위 변환·확률·조건부확률·표본 통계·오차범위·관찰 연구/실험 판단 등) 쓰고, 지문의 항목 이름·값·단위가 데이터와 정확히 같아야 한다. 마크다운 표는 쓰지 않고 이 figure 로 낸다. **정답 선택지의 수치는 반드시 figure 의 실제 셀 값이거나 그 값들의 단순 파생 통계(합·차·평균·비율 등, 계산 과정을 answer_rationale 에 명시)여야 한다 — 자료에 없는 수치를 정답에 쓰지 않는다. 오답 선택지의 수치도 임의로 지어내지 말고 자료를 잘못 읽은 값(다른 행/열, 인접 셀, 반대 방향 변화 등)이어야 한다 — 정답과 오답 모두 figure 로 되짚어 확인 가능해야 한다.**",
        ...
        : skill === "inferences"
          ...
          : skill === "central_ideas_details"
            ...
            : skill === "command_of_evidence_text"
              ...
              : "Text 1과 Text 2 사이에 실제로 성립하는 구체적 관계(동의/반박/보완 등)를 하나의 명제로 서술한 문장 — 질문을 다른 말로 옮겨 적은 것이 아니라, " +
                "두 지문을 합쳐 읽었을 때 나오는 결론 그 자체여야 한다. " +
                '나쁜 예(질문 재진술, 금지): "Text 2의 저자가 Text 1의 주장에 어떻게 반응할지". ' +
                '좋은 예(합성된 명제): "Text 2는 Text 1의 총계 데이터가 소규모 업체의 피해를 가린다고 반박한다." ' +
                "반드시 두 텍스트 중 최소 하나를 구체적으로 지칭하고, 그 입장·주장·데이터가 서로 어떻게 관련되는지(동의/반박/보완/한정 등) 명시한다.",
  },
  evidence_span: {
    type: "string",
    description:
...
질문 형식(2026-09-18, hard 재검증에서 발견) — 이 세부 기술은 반드시 "Which choice most effectively uses data from the table/graph to …?"처럼 물음표로 끝나는 완전한 의문문만 쓴다. boundaries/form_structure_sense/transitions/inferences처럼 "…complete the statement below.\n\nsince ______"식 빈칸 완성형으로 쓰지 않는다 — question 필드에 "since ______" 같은 미완성 어구만 넣거나 실제 의문문을 passage 쪽에 흘려 넣지 않는다.`;
            ...
            problems: {
              ...
              items: {
                ...
                properties: {
                  ...
                  explanation_en: {
                    type: "string",
                    description: "explanation과 같은 내용을 영어로 옮긴 해설(직역이 아니라 자연스러운 영어 설명체로).",
                  },
                ...
                required: [
                  ...(difficulty === "hard" ? ["passage", "question", "explanation", "explanation_en", "design"] : ["passage", "question", "explanation", "explanation_en"]),
...
언어: 문항(지문·질문·선택지·SPR 정답)은 실제 SAT/AP 시험과 같이 **영어**로 쓴다. 해설(explanation)은 한국어로 쓰고, explanation_en에 같은 해설을 영어로도 써준다(두 언어 모두 같은 근거·논리를 담아야 한다).
...
은행에 들어갑니다. 실전 SAT/AP 시험에 나올 법한 퀄리티로 만들어주세요.`,
  ...
  return kept.map((p) => {
    ...
    explanationEn: typeof p.explanation_en === "string" && p.explanation_en.trim() ? p.explanation_en.trim() : null,
    difficulty,
