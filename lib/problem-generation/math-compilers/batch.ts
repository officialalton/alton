...
/** 계산형 컴파일러 한 문항 시도 — 모델 생성 → 결정적 검사. 통과하면 GeneratedProblem 모양으로 돌려준다. */
function attemptOne(skillCode: MathCompilerSkill, difficulty: LinearTwoVarDifficulty): { ok: true; problem: GeneratedProblem; quality: QualityRecord } | { ok: false; reason: string } {
...
export async function runMathCompilerBatch(params: MathCompilerBatchParams): Promise<PipelineResult> {
  ...
  while (candidatesEvaluated < batchTarget || accepted.length < requested) {
    ...
    const outcome = attemptOne(params.skillCode, params.difficulty);
    if (!outcome.ok) {
  ...
  const stats: PipelineResult["stats"] = {
    ...
    timeMs: { total: Date.now() - start, generate: 0, figure: 0, contractRepair: 0, answerExplanationCheck: 0, review: 0, regenerate: 0 },
  };
  console.log(JSON.stringify({ event: "math_compiler_batch_timing", skillCode: params.skillCode, requested, batchTarget, candidatesEvaluated, accepted: accepted.length, shortfall, stoppedReason, totalMs: stats.timeMs.total }));
