...
function renderPlane(spec: CoordinatePlaneSpec): string {
  ...
  const out: string[] = [];
  out.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="coordinate plane" font-family="system-ui, sans-serif" font-size="11">`);
  // grid
  for (let x = Math.ceil(x0 / xStep) * xStep; x <= x1 + 1e-9; x += xStep) {
    out.push(`<line x1="${fmt(sx(x))}" y1="${PAD}" x2="${fmt(sx(x))}" y2="${H - PAD}" stroke="#e5e7eb" stroke-width="1"/>`);
  }
  for (let y = Math.ceil(y0 / yStep) * yStep; y <= y1 + 1e-9; y += yStep) {
    out.push(`<line x1="${PAD}" y1="${fmt(sy(y))}" x2="${W - PAD}" y2="${fmt(sy(y))}" stroke="#e5e7eb" stroke-width="1"/>`);
  }
  // axes (at 0 if in range, else at edge)
  const axX = x0 <= 0 && x1 >= 0 ? 0 : x0;
  const axY = y0 <= 0 && y1 >= 0 ? 0 : y0;
  out.push(`<line x1="${PAD}" y1="${fmt(sy(axY))}" x2="${W - PAD}" y2="${fmt(sy(axY))}" stroke="#111" stroke-width="1.4" marker-end="url(#ax)"/>`);
  out.push(`<line x1="${fmt(sx(axX))}" y1="${H - PAD}" x2="${fmt(sx(axX))}" y2="${PAD}" stroke="#111" stroke-width="1.4" marker-end="url(#ax)"/>`);
  ...
  // ticks + labels
  for (let x = Math.ceil(x0 / xStep) * xStep; x <= x1 + 1e-9; x += xStep) {
    if (Math.abs(x - axX) < 1e-9 && axX === 0) continue;
    out.push(`<text x="${fmt(sx(x))}" y="${fmt(sy(axY) + 14)}" text-anchor="middle" fill="#374151">${fmt(x)}</text>`);
  }
  for (let y = Math.ceil(y0 / yStep) * yStep; y <= y1 + 1e-9; y += yStep) {
    if (Math.abs(y - axY) < 1e-9 && axY === 0) continue;
    out.push(`<text x="${fmt(sx(axX) - 6)}" y="${fmt(sy(y) + 4)}" text-anchor="end" fill="#374151">${fmt(y)}</text>`);
  }
  out.push(`<text x="${W - PAD + 4}" y="${fmt(sy(axY) - 6)}" fill="#111" font-style="italic">${esc(spec.xLabel ?? "x")}</text>`);
  out.push(`<text x="${fmt(sx(axX) + 6)}" y="${PAD - 4}" fill="#111" font-style="italic">${esc(spec.yLabel ?? "y")}</text>`);
...
function renderPlaneItem(
  ...
  return `<path d="${d.trim()}" fill="none" stroke="${color}" stroke-width="2"/>${lastVisible ? label(lastVisible[0], lastVisible[1], item.label) : ""}`;
}
  ...
  for (const sh of spec.shapes) {
    if (sh.kind === "polygon") {
      ...
      sh.points.forEach(([x, y], i) => {
        ...
        if (al) out.push(text(sx(x) + (cx - x > 0 ? 22 : -22), sy(y) + (cy - y > 0 ? -14 : 18), al.text, "middle", false));
