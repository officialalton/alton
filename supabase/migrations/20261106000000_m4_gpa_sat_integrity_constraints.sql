...
-- =========================================================================
-- 0) 정식 데이터 마이그레이션(2026-09-06 후속 추가) — 과거 "0=미입력" 저장 방식의
--    잔재를 "SAT 미입력=null" 확정 정책에 맞게 정규화한다. 이 파일이 아직
--    반영되지 않은 환경(예: 비운영 Supabase 프로젝트)에 실제로 sat_score=0인
--    행이 남아 있어 아래 CHECK 제약(400~1600) 추가 시 위반이 발생함을 확인했다
--    (읽기 전용 점검, 2026-09-06: sat_score=0인 행 2건, 그 외 범위 위반 없음).
--    이는 실제 0점이 아니라 "미입력"을 뜻하던 구현 방식의 흔적이므로(문서 상단의
--    확정 정책 4번과 동일 원칙), 제약을 추가하기 전에 0을 null로 변환한다 —
--    임의의 다른 값으로 채우거나 0이 아닌 범위 밖 값을 건드리지 않는다.
